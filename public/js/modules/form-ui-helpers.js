/**
 * 🎨 MÓDULO DE UI HELPERS PARA FORMULARIOS
 * Funciones reutilizables para manejo de interfaz de usuario en formularios
 ...
 */
export function showLoadingState(form, message = 'Enviando...') {
 ...
 */
export function updateLoadingState(form, message) {
    const submitButton = form.querySelector('button[type="submit"]');
    if (!submitButton) return;
 ...
 * @param {HTMLFormElement} form - Formulario
 */
export function hideLoadingState(form) {
    const submitButton = form.querySelector('button[type="submit"]');
    if (!submitButton) return;
 ...
 */
export function showSuccess(form, message = null) {
 ...
 */
export function showVerificationPopup(result) {
    ...
    if (!popup.textContent) { // Solo si usamos el popup completo
        popup.style.cssText = `
            position: fixed;
            top: 0;
            left: 0;
 ...
 */
function addVerificationPopupStyles() {
 ...
 * @param {HTMLFormElement} form - Formulario
 */
export function addSecurityBadge(form) {
 ...
 */
export async function showConfirmDialog(warning) {
 ...
 * @param {HTMLFormElement} form - Formulario
 * @param {string} fieldName - Nombre del campo honeypot (default: '_gotcha')
 */
export function addHoneypot(form, fieldName = '_gotcha') {
    // Verificar si ya existe
