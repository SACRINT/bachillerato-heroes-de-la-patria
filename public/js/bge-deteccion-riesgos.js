...
class BGEDeteccionRiesgos {
    ...
    async calculateAcademicRisk(student) {
        ...
        let gradeRisk = Math.max(0, (7.0 - avgGrade) / 7.0);

        // Riesgo basado en asistencia
        let attendanceRisk = Math.max(0, (0.85 - attendance) / 0.85);

        // Riesgo basado en tareas
        let assignmentRisk = Math.max(0, (0.8 - assignments) / 0.8);
    ...
    async calculateDropoutRisk(student) {
        ...
        let attendanceRisk = Math.max(0, (0.75 - attendance) / 0.75);
        let supportRisk = Math.max(0, (0.6 - parentSupport) / 0.6);
        let economicRisk = Math.max(0, (0.5 - economicStatus) / 0.5);
        let familyRisk = Math.min(1.0, previousDropouts * 0.2);
    ...
    async calculateEmotionalRisk(student) {
        ...
        let stressRisk = stressLevel;
        let socialRisk = Math.max(0, (0.5 - socialConnection) / 0.5);
        let communicationRisk = Math.max(0, (0.4 - communicationWithTeachers) / 0.4);
    ...
    async calculateSocialRisk(student) {
        ...
        let peerRisk = Math.max(0, (0.6 - peerRelationships) / 0.6);
        let integrationRisk = Math.max(0, (0.5 - groupIntegration) / 0.5);
        let conflictRisk = Math.min(1.0, conflictHistory * 0.3);
    ...
    async calculateBehavioralRisk(student) {
        ...
        let disciplineRisk = Math.min(1.0, disciplineIssues * 0.2);
        let complianceRisk = Math.max(0, (0.8 - ruleCompliance) / 0.8);
        let aggressionRisk = Math.min(1.0, aggressionIncidents * 0.3);
    ...
    async calculateFamilyRisk(student) {
        ...
        let supportRisk = Math.max(0, (0.6 - familySupport) / 0.6);
        let stabilityRisk = Math.max(0, (0.7 - homeStability) / 0.7);
        let involvementRisk = Math.max(0, (0.5 - parentInvolvement) / 0.5);
