...
class JobPortal {
    ...
    renderFeaturedJobs() {
        ...
        if (featuredJobs.length === 0) {
            container.innerHTML = DOMPurify.sanitize('<div class="col-12 text-center text-muted">No hay empleos destacados disponibles</div>');
            return;
        }
    ...
    toggleSaveJob(jobId, button) {
        ...
        if (index === -1) {
            ...
            button.innerHTML = DOMPurify.sanitize('<i class="fas fa-bookmark text-warning"></i>');
            button.classList.remove('btn-outline-warning');
            button.classList.add('btn-warning');
            this.showAlert('Empleo guardado exitosamente', 'success');
        } else {
            this.savedJobs.splice(index, 1);
            button.innerHTML = DOMPurify.sanitize('<i class="fas fa-bookmark"></i>');
...
// Inyectar estilos
document.head.insertAdjacentHTML('beforeend', DOMPurify.sanitize(sanitizeHTML(jobPortalStyles)));
