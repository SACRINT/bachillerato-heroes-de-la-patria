...
class BGENotificationAdmin {
    ...
    loadScheduledNotifications() {
        ...
        if (this.scheduledNotifications.length === 0) {
            container.innerHTML = sanitizeHTML('<p class="no-items">No hay notificaciones programadas</p>');
            return;
        }
    ...
    loadNotificationHistoryDisplay() {
        ...
        if (this.notificationHistory.length === 0) {
            container.innerHTML = sanitizeHTML('<p class="no-items">No hay historial de notificaciones</p>');
            return;
        }
