...
class AdvancedLazyLoader {
    ...
    async loadContent(element) {
        ...
        try {
            ...
            element.innerHTML = DOMPurify.sanitize( DOMPurify.sanitize('<p class="text-muted">Error cargando contenido</p>', 'simple'));
        }
    }
    ...
    async loadNoticias(element) {
        try {
            ...
            element.innerHTML = DOMPurify.sanitize( DOMPurify.sanitize(this.renderNoticias(noticias.slice(0, 3)), 'ugc'));
    ...
    async loadEventos(element) {
        try {
            ...
            element.innerHTML = DOMPurify.sanitize( DOMPurify.sanitize(this.renderEventos(eventos.slice(0, 3)), 'ugc'));
            element.classList.add('content-loaded');
