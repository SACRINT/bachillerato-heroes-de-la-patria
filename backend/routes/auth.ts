...
interface RequestWithUser extends Request {
    user?: {
        id: number;
        userId?: number; // Algunos middlewares usan userId, otros id
        username?: string;
        email?: string;
        role: string;
        permissions?: string[];
...
router.post('/login', loginLimiter, loginValidation, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        debugLog.error('AUTH', '❌ Error en login', sanitizeError(error as Error, 'auth'));
...
router.post('/verify-2fa', loginLimiter, [
    ...
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) { res.status(400).json({ success: false, error: 'Datos de entrada inválidos', details: errors.array() }); return; }
        ...
        debugLog.error('AUTH', '❌ Error en verificación 2FA', sanitizeError(error as Error, 'auth'));
 ...
 */
router.post('/2fa/enable', authenticateToken, async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        ...
        debugLog.error('AUTH', '❌ Error al habilitar 2FA', sanitizeError(error as Error, 'auth'));
 ...
 */
router.post('/2fa/disable', authenticateToken, async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        const userId = req.user!.userId || req.user!.id;
        ...
        debugLog.error('AUTH', '❌ Error al deshabilitar 2FA', sanitizeError(error as Error, 'auth'));
 ...
 */
router.post('/2fa/regenerate-backup-codes', authenticateToken, async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        const userId = req.user!.userId || req.user!.id;
        ...
        debugLog.error('AUTH', '❌ Error al regenerar backup codes', sanitizeError(error as Error, 'auth'));
 ...
 */
router.get('/2fa/status', authenticateToken, async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        const userId = req.user!.userId || req.user!.id;
        const isEnabled = await twoFactorService.isEnabled(userId);
        res.json({ success: true, enabled: isEnabled });
    } catch (error) {
        debugLog.error('AUTH', '❌ Error al verificar estado 2FA', sanitizeError(error as Error, 'auth'));
...
router.post('/2fa/verify-setup', authenticateToken, [
    ...
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) { res.status(400).json({ success: false, error: 'Datos inválidos', details: errors.array() }); return; }
        ...
        debugLog.error('AUTH', '❌ Error verificando código de setup', sanitizeError(error as Error, 'auth'));
...
router.post('/webauthn/register/options', authenticateToken, async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        const userId = req.user!.userId || req.user!.id;
        const user = req.user!;
        debugLog.log('WEBAUTHN', `Generando opciones de registro para userId=${userId}`);
        const result = await webauthnService.generateRegistrationOptions(userId, user.username || user.name || 'User', user.email);
        res.json(result);
    } catch (error) {
        debugLog.error('WEBAUTHN', '❌ Error generando opciones de registro', sanitizeError(error as Error, 'webauthn'));
        res.status(500).json({ success: false, error: 'Error al generar opciones de registro', message: (error as Error).message });
    }
});
...
router.post('/webauthn/register/verify', authenticateToken, [
    ...
    try {
        ...
        const result = await webauthnService.verifyRegistrationResponse(userId, response, deviceName || 'Dispositivo Biométrico');
        res.json({ success: true, message: 'Dispositivo biométrico registrado exitosamente', credentialId: result.credentialId });
    } catch (error) {
        debugLog.error('WEBAUTHN', '❌ Error verificando registro', sanitizeError(error as Error, 'webauthn'));
        res.status(400).json({ success: false, error: 'Error al verificar registro', message: (error as Error).message });
    }
});
...
router.post('/webauthn/authenticate/options', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        debugLog.error('WEBAUTHN', '❌ Error generando opciones de autenticación', sanitizeError(error as Error, 'webauthn'));
        res.status(500).json({ success: false, error: 'Error al generar opciones de autenticación', message: (error as Error).message });
    }
});
...
router.post('/webauthn/authenticate/verify', loginLimiter, [
    ...
    try {
        ...
        const verificationResult = await webauthnService.verifyAuthenticationResponse(response, userId || null);

        if (!verificationResult.verified) { res.status(401).json({ success: false, error: 'Autenticación biométrica fallida' }); return; }
        ...
        debugLog.error('WEBAUTHN', '❌ Error en autenticación biométrica', sanitizeError(error as Error, 'webauthn'));
        res.status(401).json({ success: false, error: 'Error en la autenticación', message: (error as Error).message });
    }
});

router.get('/webauthn/credentials', authenticateToken, async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        const userId = req.user!.userId || req.user!.id;
        ...
        debugLog.error('WEBAUTHN', '❌ Error listando credenciales', sanitizeError(error as Error, 'webauthn'));
        res.status(500).json({ success: false, error: 'Error al listar dispositivos', credentials: [] });
    }
});

router.delete('/webauthn/credentials/:id', authenticateToken, async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        const userId = req.user!.userId || req.user!.id;
        const credentialId = parseInt(req.params.id);
        if (isNaN(credentialId)) { res.status(400).json({ success: false, error: 'ID de credencial inválido' }); return; }

        const deleted = await webauthnService.deleteCredential(credentialId, userId);
        if (!deleted) { res.status(404).json({ success: false, error: 'Credencial no encontrada' }); return; }
        ...
        debugLog.error('WEBAUTHN', '❌ Error eliminando credencial', sanitizeError(error as Error, 'webauthn'));
...
router.post('/refresh', refreshLimiter, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!refreshToken) { res.status(401).json({ success: false, error: 'Token de refresh requerido', message: 'Debes proporcionar un token de refresh válido' }); return; }
        ...
        debugLog.error('AUTH', 'Error renovando token', sanitizeError(error as Error, 'auth'));
        res.status(403).json({ success: false, error: 'Token de refresh inválido', message: (error as Error).message });
    }
});
 ...
 */
router.post('/logout', authenticateToken, async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        ...
        debugLog.error('AUTH', '❌ Error en logout', sanitizeError(error as Error, 'auth'));
        res.status(500).json({ success: false, error: 'Error cerrando sesión', message: (error as Error).message });
    }
});
 ...
 */
router.post('/register', authenticateToken, requireAdmin, registerLimiter, registerValidation, async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) { res.status(400).json({ success: false, error: 'Datos de entrada inválidos', details: errors.array() }); return; }
        ...
        debugLog.error('AUTH', '❌ Error registrando usuario', sanitizeError(error as Error, 'auth'));
 ...
 */
router.get('/profile', authenticateToken, async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        const userProfile = await authService.getUserProfile(req.user!.id);
        res.json({ success: true, message: 'Perfil obtenido exitosamente', user: userProfile });
    } catch (error) {
        debugLog.error('AUTH', '❌ Error obteniendo perfil', sanitizeError(error as Error, 'auth'));
        res.status(404).json({ success: false, error: 'Usuario no encontrado', message: (error as Error).message });
    }
});
 ...
 */
router.put('/change-password', authenticateToken, passwordChangeValidation, async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) { res.status(400).json({ success: false, error: 'Datos de entrada inválidos', details: errors.array() }); return; }
        ...
        debugLog.error('AUTH', '❌ Error cambiando contraseña', sanitizeError(error as Error, 'auth'));
        const errorMessage = (error as Error).message;
 ...
 * GET /api/auth/verify
 */
router.get('/verify', authenticateToken, (req: RequestWithUser, res: Response) => {
 ...
 */
router.get('/permissions', authenticateToken, (req: RequestWithUser, res: Response) => {
    const userPermissions = authService.permissions[req.user!.role] || [];
    res.json({ success: true, permissions: userPermissions, role: req.user!.role, hasPermission: (permission: string) => authService.hasPermission(req.user!.role, permission) });
});

/**
 * POST /api/auth/check-permission
 */
router.post('/check-permission', authenticateToken, [body('permission').notEmpty().withMessage('Permiso requerido')], (req: RequestWithUser, res: Response): void => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) { res.status(400).json({ success: false, error: 'Datos inválidos', details: errors.array() }); return; }
 ...
 */
router.post('/invalidate-user-sessions', authenticateToken, requireAdmin, [body('userId').isInt().withMessage('ID de usuario requerido')], async (req: RequestWithUser, res: Response): Promise<void> => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) { res.status(400).json({ success: false, error: 'Datos inválidos', details: errors.array() }); return; }
        const { userId } = req.body;
        ...
        debugLog.error('AUTH', '❌ Error invalidando sesiones', sanitizeError(error as Error, 'auth'));
...
const requestRegistrationValidation = [
    body('fullName').trim().isLength({ min: 5, max: 200 }).matches(/^[a-záéíóúñA-ZÁÉÍÓÚÑ\s]+$/).withMessage('Nombre completo debe tener entre 5 y 200 caracteres y solo contener letras'),
    body('email').isEmail().normalizeEmail().withMessage('Email válido requerido').custom((value: string) => {
        const allowedDomains = ['@bge.edu.mx', '@bgeheroespatria.edu.mx', '@heroespatria.edu.mx'];
        if (!allowedDomains.some(domain => value.endsWith(domain))) { throw new Error('Solo se permiten correos institucionales'); }
        return true;
    }),
    body('requestedRole').isIn(['docente', 'estudiante', 'administrativo']).withMessage('Rol debe ser: docente, estudiante o administrativo'),
    body('reason').trim().isLength({ min: 50, max: 1000 }).withMessage('El motivo debe tener entre 50 y 1000 caracteres'),
    body('phone').optional().matches(/^[0-9]{10}$/).withMessage('Teléfono debe tener 10 dígitos')
];
...
const RegistrationHelpers = {
    ...
    async hasPendingRequest(email: string) {
        const data = await this.readRegistrationRequests();
        return data.requests.some((req: any) => req.email.toLowerCase() === email.toLowerCase() && req.status === 'pending');
    }
};

router.post('/request-registration', registrationRequestLimiter, requestRegistrationValidation, async (req: Request, res: Response): Promise<void> => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) { res.status(400).json({ success: false, error: 'Datos de entrada inválidos', details: errors.array() }); return; }

        const { fullName, email, requestedRole, reason, phone } = req.body;
        if (await RegistrationHelpers.hasPendingRequest(email)) { res.status(409).json({ success: false, error: 'Solicitud duplicada', message: 'Ya existe una solicitud pendiente.' }); return; }

        try {
            const jsonUsers = await authService.loadUsersFromJson();
            if (jsonUsers.some((u: any) => u.email.toLowerCase() === email.toLowerCase())) { res.status(409).json({ success: false, error: 'Email ya registrado', message: 'Este email ya está registrado.' }); return; }
        } catch (error) { debugLog.log('AUTH', `⚠️ No se pudo verificar usuarios existentes: ${(error as Error).message}`); }
        ...
        debugLog.error('AUTH', '❌ Error procesando solicitud de registro', sanitizeError(error as Error, 'auth'));
...
router.post('/google', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!credential) { res.status(400).json({ success: false, error: 'Token de Google requerido' }); return; }
        ...
        try {
            ...
            debugLog.error('AUTH', '[GOOGLE-AUTH] Token inválido', sanitizeError(error as Error, 'auth'));
            res.status(401).json({ success: false, error: 'Token de Google inválido', message: (error as Error).message }); return;
        }
        ...
        debugLog.error('AUTH', '❌ [GOOGLE-AUTH] Error procesando autenticación', sanitizeError(error as Error, 'auth'));
        res.status(500).json({ success: false, error: 'Error procesando autenticación con Google', message: (error as Error).message });
    }
});
...
const publicRegisterValidation = [
    ...
    body('apellido_paterno').isLength({ min: 2, max: 100 }).withMessage('Apellido paterno entre 2 y 100 caracteres')
];

router.post('/public-register', registerLimiter, publicRegisterValidation, async (req: Request, res: Response): Promise<void> => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) { res.status(400).json({ success: false, error: 'Datos de entrada inválidos', details: errors.array() }); return; }
        ...
        if (existingUser) {
            if (existingUser.email_verified) {
                res.status(409).json({ success: false, error: 'Email ya registrado', message: 'Este email ya está registrado. Intenta iniciar sesión o recuperar tu contraseña.' }); return;
            } else {
        ...
        debugLog.error('AUTH', `[PUBLIC-REGISTER] Error: ${(error as Error).message}`);
...
router.post('/verify-email', async (req: Request, res: Response): Promise<void> => {
    try {
        const { token } = req.body;
        if (!token) { res.status(400).json({ success: false, error: 'Token requerido', message: 'Debes proporcionar el token de verificación.' }); return; }
        ...
        if (!verificationData) { res.status(400).json({ success: false, error: 'Token inválido', message: 'El enlace de verificación no es válido o ya fue utilizado.' }); return; }

        if (new Date() > new Date(verificationData.expires_at)) { res.status(400).json({ success: false, error: 'Token expirado', message: 'El enlace de verificación ha expirado.' }); return; }
        ...
        debugLog.error('AUTH', `[VERIFY-EMAIL] Error: ${(error as Error).message}`);
        res.status(500).json({ success: false, error: 'Error en la verificación', message: 'No se pudo verificar el email.' });
    }
});

router.post('/resend-verification', registerLimiter, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!user) { res.json({ success: true, message: 'Si el email existe y no está verificado, recibirás un nuevo enlace.' }); return; }
        if (user.email_verified) { res.status(400).json({ success: false, error: 'Email ya verificado', message: 'Este email ya está verificado.' }); return; }
        ...
        debugLog.error('AUTH', `[RESEND-VERIFICATION] Error: ${(error as Error).message}`);
