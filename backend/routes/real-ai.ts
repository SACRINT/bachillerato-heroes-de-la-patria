...
// @ts-ignore
import { getRealAIService } from '../services/realAIService';
// @ts-ignore
...
router.get('/health', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        debugLog.error('REAL_AI', '❌ Error en health check Real AI:', sanitizeError(error, 'real-ai'));
        res.status(500).json({
            status: 'error',
 ...
 */
router.post('/process', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        const {
            message,
            context,
        ...
        debugLog.log('REAL_AI', `🤖 Procesando solicitud de IA para usuario: ${user.email}`);
        ...
        debugLog.error('REAL_AI', '❌ Error procesando con IA real:', sanitizeError(error, 'real-ai'));
        res.status(500).json({
            success: false,
...
router.post('/chat', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const user = (req as any).user;
        debugLog.log('REAL_AI', `💬 Chat IA para usuario: ${user.email}`);
        ...
        debugLog.error('REAL_AI', '❌ Error en chat IA:', sanitizeError(error, 'real-ai'));
        res.status(500).json({
            success: false,
...
router.post('/analyze', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const user = (req as any).user;
        debugLog.log('REAL_AI', `📊 Análisis IA para usuario: ${user.email}, tipo: ${analysisType}`);
        ...
        const systemPrompt = analysisPrompts[analysisType as string] || analysisPrompts.general;
        ...
        debugLog.error('REAL_AI', '❌ Error en análisis IA:', sanitizeError(error, 'real-ai'));
        res.status(500).json({
            success: false,
...
router.get('/providers', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        debugLog.error('REAL_AI', '❌ Error obteniendo info de proveedores:', sanitizeError(error, 'real-ai'));
        res.status(500).json({
            success: false,
 ...
 */
router.post('/reload', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        const user = (req as any).user;
        ...
        debugLog.log('REAL_AI', `🔄 Recargando configuración IA por: ${user.email}`);
        ...
        debugLog.error('REAL_AI', '❌ Error recargando configuración IA:', sanitizeError(error, 'real-ai'));
        res.status(500).json({
            success: false,
...
router.get('/stats', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        debugLog.error('REAL_AI', '❌ Error obteniendo estadísticas IA:', sanitizeError(error, 'real-ai'));
        res.status(500).json({
            success: false,
