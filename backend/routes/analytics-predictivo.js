...
router.get('/health', async (req, res) => {
    try {
        const health = {
            ...
            systemInfo: {
                modelsEnabled: Object.keys(ANALYTICS_CONFIG.models).filter((model) => ANALYTICS_CONFIG.models[model].enabled),
        ...
        debug_logger_1.debugLog.error('analytics-predictivo', 'Analytics health check error', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
        res.status(500).json({
            status: 'error',
...
router.post('/risk-prediction', async (req, res) => {
    ...
    try {
        ...
        debug_logger_1.debugLog.log('analytics-predictivo', `🎯 [ANALYTICS-API] Generando predicción de riesgo para estudiante ${studentId}`);
        ...
        // Log del resultado
        debug_logger_1.debugLog.log('analytics-predictivo', `✅ [ANALYTICS-API] Predicción de riesgo generada en ${processingTime}ms - Riesgo: ${riskPrediction.riskLevel} (${(riskPrediction.overallRisk * 100).toFixed(1)}%)`);
        res.json(result);
    }
    catch (error) {
        const processingTime = Date.now() - startTime;
        debug_logger_1.debugLog.error('analytics-predictivo', 'Risk prediction error', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
        res.status(500).json({
...
router.post('/performance-trends', async (req, res) => {
    ...
    try {
        ...
        debug_logger_1.debugLog.log('analytics-predictivo', `📈 [ANALYTICS-API] Generando análisis de tendencias para estudiante ${studentId}`);
        ...
        debug_logger_1.debugLog.log('analytics-predictivo', `✅ [ANALYTICS-API] Análisis de tendencias generado en ${processingTime}ms`);
        res.json(result);
    }
    catch (error) {
        const processingTime = Date.now() - startTime;
        debug_logger_1.debugLog.error('analytics-predictivo', 'Performance trends error', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
        res.status(500).json({
...
router.get('/dashboard', async (req, res) => {
    try {
        const { grade, group, timeRange = 'current', includeAlerts = true, includeRecommendations = true } = req.query;
        debug_logger_1.debugLog.log('analytics-predictivo', '📊 [ANALYTICS-API] Generando dashboard de analytics...');
        ...
        debug_logger_1.debugLog.error('analytics-predictivo', 'Dashboard generation error', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
        res.status(500).json({
...
router.get('/alerts', async (req, res) => {
    try {
        const { severity, type, studentId, limit = '50', offset = '0' } = req.query;
        ...
        debug_logger_1.debugLog.error('analytics-predictivo', 'Alerts retrieval error', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
        res.status(500).json({
...
router.post('/alerts', async (req, res) => {
    try {
        const { studentId, type, severity, message, data = {}, autoResolve = true } = req.body;
        // Validación
        ...
        debug_logger_1.debugLog.log('analytics-predictivo', `🚨 [ANALYTICS-API] Alerta creada: ${type} para estudiante ${studentId} (${severity})`);
        res.json({
            success: true,
            alert: alert,
        ...
        debug_logger_1.debugLog.error('analytics-predictivo', 'Alert creation error', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
        res.status(500).json({
...
router.get('/stats', async (req, res) => {
    try {
        ...
        debug_logger_1.debugLog.error('analytics-predictivo', 'Stats retrieval error', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
        res.status(500).json({
...
router.get('/group-analysis', async (req, res) => {
    try {
        const { groups, subjects, timeRange = 'semester', metrics = 'all' } = req.query;
        const groupsArray = groups ? groups.split(',') : ['all'];
        const subjectsArray = subjects ? subjects.split(',') : ['all'];
        const metricsArray = metrics.split(',');
        debug_logger_1.debugLog.log('analytics-predictivo', `👥 [ANALYTICS-API] Generando análisis comparativo de grupos: ${groupsArray.join(', ')}`);
        ...
        debug_logger_1.debugLog.error('analytics-predictivo', 'Group analysis error', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
        res.status(500).json({
...
router.post('/batch-predictions', async (req, res) => {
    try {
        ...
        debug_logger_1.debugLog.log('analytics-predictivo', `📊 [ANALYTICS-API] Procesando batch de predicciones para ${studentIds.length} estudiantes`);
        const results = [];
        const errors = [];
        ...
        res.json({
            ...
            // errors: errors.length, // Duplicate key issue if kept
            results: results,
            errors: errors,
            metadata: {
        ...
        debug_logger_1.debugLog.error('analytics-predictivo', 'Batch predictions error', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
        res.status(500).json({
...
async function generateRiskPrediction(student, options = {}) {
    try {
        ...
        debug_logger_1.debugLog.error('analytics-predictivo', 'Error generating risk prediction', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
...
async function generatePerformanceTrends(student, timeRange, options = {}) {
    try {
        ...
        debug_logger_1.debugLog.error('analytics-predictivo', 'Error generating performance trends', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
...
async function generateRealTimeDashboard(filters = {}) {
    try {
        ...
        debug_logger_1.debugLog.error('analytics-predictivo', 'Error generating dashboard', (0, sanitized_errors_1.sanitizeError)(error, 'analytics-predictivo'));
...
function calculateAcademicRisk(features) {
    let risk = 0;
    if (features.overallGrade < 6)
        risk += 0.6;
    else if (features.overallGrade < 7)
        risk += 0.4;
    else if (features.overallGrade < 8)
        risk += 0.2;
    // Agregar variabilidad en las materias
    if (features.subjectVariance > 1.5)
        risk += 0.2;
...
function calculateSocialRisk(features) {
    let socialRisk = 0;
    if (features.workHours > 20)
        socialRisk += 0.5;
    else if (features.workHours > 10)
        socialRisk += 0.2;
...
function identifyContributingFactors(riskScores, features) {
    const factors = [];
    if (riskScores.academic > 0.5)
        factors.push('Calificaciones bajas');
    if (riskScores.attendance > 0.5)
        factors.push('Ausentismo elevado');
    if (riskScores.behavior > 0.5)
        factors.push('Problemas de comportamiento');
    return factors;
}
...
async function createAlert(alertData) { return alertData; }
async function processAlert(alert) { }
function calculateCacheHitRate() { return 0.85; }
function calculateAverageResponseTime() { return 150; }
async function generateGroupComparison(params) { return { dataPoints: 0 }; }
...
//# sourceMappingURL=analytics-predictivo.js.map