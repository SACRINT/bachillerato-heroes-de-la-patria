 ...
 */
router.get('/boleta/:studentId', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        const authReq = req as AuthenticatedRequest;
        const { studentId } = req.params;
        const ciclo = (req.query.ciclo as string) || '2025-2026';
        ...
        const grades = await executeQuery(`
            ...
            ORDER BY m.nombre
        `, [studentId, ciclo]) as any[];

        // Calculate average
        const validGrades = grades.filter((g: any) => g.promediofinal != null);
        const promedioGeneral = validGrades.length > 0
            ? (validGrades.reduce((sum: number, g: any) => sum + parseFloat(g.promediofinal), 0) / validGrades.length).toFixed(1)
            : 'N/A';
        ...
        debugLog.error('REPORTS', 'Error generating boleta', sanitizeError(error as Error, 'REPORTS'));
 ...
 */
router.get('/asistencia/estudiante/:studentId', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        const { studentId } = req.params;
        ...
        `, [studentId, startDate.toISOString(), endDate.toISOString()]) as any[];

        // Calculate stats
        const totalClases = records.length;
        const asistencias = records.filter((r: any) => r.presente).length;
        const faltas = totalClases - asistencias;
        const faltasJustificadas = records.filter((r: any) => !r.presente && r.justificada).length;
        ...
        debugLog.error('REPORTS', 'Error generating attendance report', sanitizeError(error as Error, 'REPORTS'));
 ...
 */
router.get('/asistencia/clase/:classId', authenticateToken, requireRole(['docente', 'admin']), async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        `, [classId, date.toISOString()]) as any[];

        const presentes = students.filter((s: any) => s.presente).length;
        ...
        debugLog.error('REPORTS', 'Error generating class attendance PDF', sanitizeError(error as Error, 'REPORTS'));
...
router.get('/available', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    ...
    if (['estudiante', 'padre'].includes(role)) {
        reports.push(
            { id: 'boleta', name: 'Boleta de Calificaciones', endpoint: '/api/reports/boleta/:studentId' },
            { id: 'asistencia', name: 'Reporte de Asistencia', endpoint: '/api/reports/asistencia/estudiante/:studentId' }
        );
    }

    if (['docente', 'admin'].includes(role)) {
        reports.push(
            { id: 'lista-asistencia', name: 'Lista de Asistencia', endpoint: '/api/reports/asistencia/clase/:classId' }
        );
    }
