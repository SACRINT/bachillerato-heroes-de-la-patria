...
try {
    ...
    // @ts-ignore
    superAdminService = require('../services/super-admin.service.js').default;
}
...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Error obteniendo estadísticas del dashboard', message: error.message });
    }
});
/**
...
router.get('/tenants', async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Error listando tenants', message: error.message });
    }
});
/**
...
router.get('/tenants/:id', async (req, res) => {
    try {
        ...
        const code = error.message === 'Tenant no encontrado' ? 404 : 500;
...
router.post('/plans', async (req, res) => {
    try {
        ...
        res.status(400).json({ success: false, error: 'Error creando plan', message: error.message });
    }
});
router.put('/plans/:id', async (req, res) => {
    try {
        ...
        res.status(400).json({ success: false, error: 'Error actualizando plan', message: error.message });
    }
});
router.post('/tenants/:id/subscription', async (req, res) => {
    try {
        ...
        res.status(400).json({ success: false, error: 'Error asignando suscripción', message: error.message });
    }
});
