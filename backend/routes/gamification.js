 ...
 */
router.get('/profile/:userId', auth_1.authenticateToken, async (req, res, next) => {
    try {
        const authReq = req;
        ...
        debug_logger_1.debugLog.log('GAMIFICATION', `🎮 [GAMIFICATION] Obteniendo perfil de gamificación para usuario ${userId}`);
        ...
        debug_logger_1.debugLog.log('GAMIFICATION', '✅ Perfil de gamificación consultado', {
            userId: userId,
 ...
 */
router.get('/leaderboard', auth_1.authenticateToken, async (req, res, next) => {
    try {
        const { type = 'weekly', limit = '10' } = req.query;
        const limitNum = parseInt(limit);
        debug_logger_1.debugLog.log('GAMIFICATION', `🏆 [GAMIFICATION] Obteniendo leaderboard tipo: ${type}`);
        // Simular leaderboard
        const leaderboard = [];
        const users = []; // Datos simulados sin acceso a BD
        for (let i = 0; i < Math.min(limitNum, users.length + 7); i++) {
 ...
 */
router.post('/award-points', auth_1.authenticateToken, async (req, res, next) => {
    try {
        const authReq = req;
        const { activity, points, metadata } = req.body;
        debug_logger_1.debugLog.log('GAMIFICATION', `⭐ [GAMIFICATION] Otorgando ${points} puntos por actividad: ${activity}`);
        // Validar actividad
        ...
        debug_logger_1.debugLog.log('GAMIFICATION', '⭐ Puntos otorgados en gamificación', {
 ...
 */
router.get('/achievements', auth_1.authenticateToken, async (req, res, next) => {
    try {
        debug_logger_1.debugLog.log('GAMIFICATION', '🏅 [GAMIFICATION] Obteniendo lista de logros disponibles');
        const achievements = [
            {
                id: 'early_bird',
                name: 'Madrugador',
 ...
 */
router.get('/daily-challenges', auth_1.authenticateToken, async (req, res, next) => {
    try {
        debug_logger_1.debugLog.log('GAMIFICATION', '📅 [GAMIFICATION] Obteniendo desafíos diarios');
 ...
 */
router.post('/complete-challenge', auth_1.authenticateToken, async (req, res, next) => {
    try {
        const authReq = req;
        const { challengeId, evidence } = req.body;
        debug_logger_1.debugLog.log('GAMIFICATION', `🎯 [GAMIFICATION] Completando desafío: ${challengeId}`);
        ...
        debug_logger_1.debugLog.log('GAMIFICATION', '🎯 Desafío completado', {
            userId: authReq.user.id,
...
router.get('/profile/:userId', async (req, res, next) => {
    try {
        const { userId } = req.params;
        debug_logger_1.debugLog.log('GAMIFICATION', `🎮 [GAMIFICATION PUBLIC] Obteniendo perfil de gamificación para usuario ${userId}`);
...
router.get('/daily-challenges', async (req, res, next) => {
    try {
        debug_logger_1.debugLog.log('GAMIFICATION', '🎯 [GAMIFICATION PUBLIC] Obteniendo desafíos diarios');
        const challenges = [
            {
                id: 'login_daily',
                title: 'Acceso Diario',
