    ...
    try {
        ...
        if (AI_CONFIG.openai.apiKey) {
            try {
                ...
                debug_logger_1.debugLog.log('CHATBOT_IA', 'OpenAI health check failed:', error.message);
            }
        }
        // Verificar Claude
        if (AI_CONFIG.claude.apiKey && !healthStatus.availableModel) {
            try {
                ...
                debug_logger_1.debugLog.log('CHATBOT_IA', 'Claude health check failed:', error.message);
            }
        }
        ...
        debug_logger_1.debugLog.error('CHATBOT_IA', 'Health check error:', (0, sanitized_errors_1.sanitizeError)(error, 'chatbot-ia'));
        res.status(500).json({
            status: 'error',
...
router.post('/chat', async (req, res) => {
    ...
    try {
        const { message, context = {}, conversationHistory = [], systemPrompt = '', config = {} } = req.body;
        ...
        const responseTime = Date.now() - startTime;
        debug_logger_1.debugLog.error('CHATBOT_IA', 'Chat endpoint error:', (0, sanitized_errors_1.sanitizeError)(error, 'chatbot-ia'));
        // Log de error
        logChatResponse({ requestId: generateRequestId() }, null, responseTime, false, error.message);
        // Respuesta de error
        res.status(500).json({
...
async function processWithAvailableIA(params) {
    ...
    if (AI_CONFIG.openai.apiKey) {
        try {
            ...
            debug_logger_1.debugLog.log('CHATBOT_IA', 'OpenAI failed, trying Claude:', error.message);
        }
    }
    // Intentar con Claude
    if (AI_CONFIG.claude.apiKey) {
        try {
            ...
            debug_logger_1.debugLog.log('CHATBOT_IA', 'Claude failed, using local IA:', error.message);
        }
    }
    // Fallback a IA local
...
function logChatQuery(context, message) {
    debug_logger_1.debugLog.log('CHATBOT_IA', `📝 [CHAT-IA] Query from ${context.userType || 'guest'}: "${message.substring(0, 50)}..."`);
}
function logChatResponse(context, response, responseTime, success, error = null) {
    if (success) {
        debug_logger_1.debugLog.log('CHATBOT_IA', `✅ [CHAT-IA] Response sent (${responseTime}ms) - Model: ${response?.model || 'unknown'}`);
    }
    else {
        debug_logger_1.debugLog.log('CHATBOT_IA', `❌ [CHAT-IA] Response failed (${responseTime}ms) - Error: ${error}`);
    }
}
// ============================================
// OTHER ROUTES
...
router.get('/stats', async (req, res) => {
    try {
        ...
        debug_logger_1.debugLog.error('CHATBOT_IA', 'Stats endpoint error:', (0, sanitized_errors_1.sanitizeError)(error, 'chatbot-ia'));
        res.status(500).json({
...
router.post('/config', async (req, res) => {
    try {
        ...
        if (claudeKey)
            AI_CONFIG.claude.apiKey = claudeKey;
        res.json({
        ...
        debug_logger_1.debugLog.error('CHATBOT_IA', 'Config endpoint error:', (0, sanitized_errors_1.sanitizeError)(error, 'chatbot-ia'));
        res.status(500).json({
...
//# sourceMappingURL=chatbot-ia.js.map