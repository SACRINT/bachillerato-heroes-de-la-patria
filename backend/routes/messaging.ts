...
router.get('/conversations/:id', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const participantsResult = await client.query(
            `SELECT user_id, user_role, user_name, user_email, is_admin, joined_at 
        ...
        debugLog.error('messaging', 'Error al obtener conversación', sanitizeError(error as Error, 'messaging'));
 ...
 */
router.get('/conversations/:id/messages', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    const client = await pool.connect();
    try {
        const user = getUserFromToken(req as AuthenticatedRequest);
        const { id } = req.params;
        ...
        debugLog.error('messaging', 'Error al obtener mensajes', sanitizeError(error as Error, 'messaging'));
 ...
 */
router.post('/conversations/:id/mark-all-read', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    const client = await pool.connect();
    try {
        const user = getUserFromToken(req as AuthenticatedRequest);
        const { id } = req.params;
        ...
        await client.query(
            `INSERT INTO message_reads (message_id, user_id, user_role)
             ...
             ON CONFLICT (message_id, user_id, user_role) DO NOTHING`,
            [id, user.id, user.role]
        );
        ...
        debugLog.error('messaging', 'Error al marcar como leído', sanitizeError(error as Error, 'messaging'));
 ...
 */
router.post('/conversations/:id/typing', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        const user = getUserFromToken(req as AuthenticatedRequest);
        const { id } = req.params;
        ...
        debugLog.log('messaging', `👤 ${user.name} ${is_typing ? 'está escribiendo' : 'dejó de escribir'} en conversación ${id}`);
        ...
        debugLog.error('messaging', 'Error en typing indicator', sanitizeError(error as Error, 'messaging'));
