 ...
 */
router.get('/stats', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res, next) => {
    try {
        const authReq = req;
        debug_logger_1.debugLog.log('DASHBOARD', '📊 [DASHBOARD] Obteniendo estadísticas generales...');
        const users = [];
        const userStats = {
            total: users.length,
        ...
        debug_logger_1.debugLog.log('DASHBOARD', 'Estadísticas del dashboard consultadas', {
 ...
 */
router.get('/recent-activity', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res, next) => {
    try {
        debug_logger_1.debugLog.log('DASHBOARD', '🔄 [DASHBOARD] Obteniendo actividad reciente...');
        const activities = [
            {
                id: 1,
                type: 'login',
 ...
 */
router.get('/system-health', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res, next) => {
    try {
        debug_logger_1.debugLog.log('DASHBOARD', '🏥 [DASHBOARD] Verificando salud del sistema...');
        const health = {
            database: {
                status: 'healthy',
 ...
 */
router.get('/quick-actions', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res, next) => {
    try {
        const actions = [
            {
                id: 'create_user',
 ...
 */
router.post('/execute-action', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res, next) => {
    try {
        const authReq = req;
        const { actionId, parameters } = req.body;
        debug_logger_1.debugLog.log('DASHBOARD', '⚡ [DASHBOARD] Ejecutando acción:', actionId);
        const results = {
            create_user: {
                success: true,
        ...
        debug_logger_1.debugLog.log('DASHBOARD', 'Acción del dashboard ejecutada:', actionId, {
            adminId: authReq.user.id,
            actionId,
            parameters,
 ...
 */
router.get('/active-users', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res, next) => {
    try {
        debug_logger_1.debugLog.log('DASHBOARD', '👥 [DASHBOARD] Obteniendo usuarios activos...');
        ...
        const activeUsers = activeSessions.map((session, index) => {
            try {
                ...
                debug_logger_1.debugLog.error('DASHBOARD', 'Error procesando sesión:', (0, sanitized_errors_1.sanitizeError)(error, 'dashboard'));
                return null;
            }
        }).filter((user) => user !== null);
        debug_logger_1.debugLog.log('DASHBOARD', `✅ [DASHBOARD] ${activeUsers.length} usuarios activos encontrados`);
        res.json({
            success: true,
            data: activeUsers,
        ...
        debug_logger_1.debugLog.error('DASHBOARD', '❌ [DASHBOARD] Error obteniendo usuarios activos:', (0, sanitized_errors_1.sanitizeError)(error, 'dashboard'));
        next(error);
    }
});
