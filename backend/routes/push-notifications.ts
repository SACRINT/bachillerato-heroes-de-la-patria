...
router.post('/subscribe', authenticateToken, [
    ...
    try {
        ...
        `, [authReq.user.id, subscription.endpoint]) as any[];
        ...
        debugLog.error('PUSH', 'Error subscribing to push', sanitizeError(error as Error, 'PUSH'));
...
router.delete('/unsubscribe', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        debugLog.error('PUSH', 'Error unsubscribing', sanitizeError(error as Error, 'PUSH'));
...
router.post('/send', authenticateToken, requireRole(['admin', 'docente']), [
    ...
    try {
        ...
        debugLog.error('PUSH', 'Error sending push notification', sanitizeError(error as Error, 'PUSH'));
...
router.post('/broadcast', authenticateToken, requireRole(['admin']), [
    body('role').isIn(['estudiante', 'padre', 'docente', 'admin', 'all']),
    body('title').notEmpty(),
    body('body').notEmpty()
] as ValidationChain[], async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        debugLog.error('PUSH', 'Error broadcasting notification', sanitizeError(error as Error, 'PUSH'));
 ...
 * Send push notification to specific user
 */
async function sendPushToUser(userId: number, payload: NotificationPayload): Promise<{ sent: number; failed: number }> {
 ...
 */
async function broadcastPushToRole(role: string, payload: NotificationPayload): Promise<{ sent: number; failed: number }> {
    let query = `
    ...
    const subscriptions = await executeQuery(query, params) as any[];
    return await sendPushToSubscriptions(subscriptions, payload);
}
 ...
 */
async function sendPushToSubscriptions(subscriptions: any[], payload: NotificationPayload): Promise<{ sent: number; failed: number }> {
    let sent = 0;
    let failed = 0;
