...
router.post('/auth/login', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const parent: Parent = result.rows[0];
 ...
 */
router.get('/students/:studentId/grades', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    const client = await pool.connect();
 ...
 */
router.get('/students/:studentId/attendance', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    const client = await pool.connect();
