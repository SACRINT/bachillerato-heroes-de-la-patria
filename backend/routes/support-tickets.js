...
router.get('/departments', auth_1.authenticateToken, async (req, res) => {
    ...
    try {
        ...
        debug_logger_1.debugLog.error('support-tickets', 'Error al obtener departamentos', (0, sanitized_errors_1.sanitizeError)(error, 'support-tickets'));
...
router.get('/categories', auth_1.authenticateToken, async (req, res) => {
    ...
    try {
        ...
        debug_logger_1.debugLog.error('support-tickets', 'Error al obtener categorías', (0, sanitized_errors_1.sanitizeError)(error, 'support-tickets'));
...
router.get('/tickets', auth_1.authenticateToken, async (req, res) => {
    ...
    try {
        const authReq = req;
        const { status, priority, category_id, department_id, assigned_to_me, created_by_me, watching, page = '1', limit = '20', sort_by = 'created_at', sort_order = 'DESC' } = req.query;
        ...
        debug_logger_1.debugLog.error('support-tickets', 'Error al obtener tickets', (0, sanitized_errors_1.sanitizeError)(error, 'support-tickets'));
...
router.post('/tickets', auth_1.authenticateToken, upload.array('attachments', 5), async (req, res) => {
    const client = await database_1.pool.connect();
    try {
        const authReq = req;
        ...
        debug_logger_1.debugLog.error('support-tickets', 'Error al crear ticket', (0, sanitized_errors_1.sanitizeError)(error, 'support-tickets'));
