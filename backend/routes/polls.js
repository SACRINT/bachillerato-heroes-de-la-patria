...
router.post('/', async (req, res) => {
    try {
        ...
        const { title, description, type = 'single_choice', options = [], categories = [], starts_at, ends_at, allow_multiple_votes = false, show_results_before_voting = false, anonymous_voting = true, require_login = false, target_audience = 'public', featured = false, image_url, color = '#3498db' } = req.body;
        // Validaciones
        if (!title || title.trim() === '') {
            res.status(400).json({
                success: false,
