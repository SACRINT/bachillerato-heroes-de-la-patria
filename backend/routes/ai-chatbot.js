...
router.post('/message', async (req, res) => {
    try {
        ...
        let response;
        if (isAuthenticated) {
            response = await openai_service_1.default.continueConversation(userId, message, {
                language,
                includeContext
            });
        }
            ...
            response = await openai_service_1.default.generateChatResponse(message, {
                language,
                includeContext
            });
        }
...
router.get('/history', auth_1.authenticateJWT, async (req, res) => {
    try {
        ...
        const history = await openai_service_1.default.getChatHistory(userId, limit);
        res.status(200).json({
            success: true,
            history,
 ...
 */
router.delete('/history', auth_1.authenticateJWT, async (req, res) => {
    try {
        const userId = req.user.id;
 ...
 */
router.post('/faq', auth_1.authenticateJWT, (0, auth_1.requireRole)(['admin', 'administrativo']), async (req, res) => {
    try {
        ...
        const faq = await openai_service_1.default.createFAQ({
            pregunta,
            respuesta,
            categoria,
            idioma,
 ...
 */
router.put('/faq/:id', auth_1.authenticateJWT, (0, auth_1.requireRole)(['admin', 'administrativo']), async (req, res) => {
    try {
        ...
        const faq = await openai_service_1.default.updateFAQ(id, updates);
        res.status(200).json({
            success: true,
            faq,
 ...
 */
router.delete('/faq/:id', auth_1.authenticateJWT, (0, auth_1.requireRole)(['admin', 'administrativo']), async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            res.status(400).json({ error: 'invalid_id', message: 'ID inválido.' });
 ...
 */
router.get('/faqs', auth_1.authenticateJWT, (0, auth_1.requireRole)(['admin', 'administrativo', 'docente']), async (req, res) => {
    try {
        ...
        const faqs = await openai_service_1.default.getAllFAQs(filters);
        res.status(200).json({
            success: true,
            faqs,
            count: faqs.length
 ...
 */
router.get('/analytics', auth_1.authenticateJWT, (0, auth_1.requireRole)(['admin', 'administrativo']), async (req, res) => {
    try {
        ...
        const analytics = await openai_service_1.default.getChatbotAnalytics(filters);
