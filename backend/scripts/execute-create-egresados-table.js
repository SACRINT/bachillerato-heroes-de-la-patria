...
async function createEgresadosTable() {
    devLogger.log('🚀 Iniciando creación de tabla de egresados...\n');
    ...
    try {
        ...
        if (result.rows.length > 0) {
            devLogger.log('✅ Tabla encontrada: egresados');
        }
        ...
        devLogger.log(`✅ Perfiles de prueba insertados: ${statsResult.rows[0].total}`);
        ...
        profilesResult.rows.forEach((row, index) => {
            devLogger.log(`   ${index + 1}. ${row.egresado_id} - ${row.nombre_completo}`);
            devLogger.log(`      Carrera: ${row.carrera_tecnica}`);
            devLogger.log(`      Estado: ${row.estado_perfil}\n`);
        });

        devLogger.log('🎉 ¡Tabla de egresados creada exitosamente!');
        devLogger.log('🔧 Ahora puedes usar el sistema de perfiles profesionales');
