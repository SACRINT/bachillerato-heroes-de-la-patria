...
console.log(`DATABASE_URL empieza con: ${process.env.DATABASE_URL ? process.env.DATABASE_URL.substring(0, 20) + '...' : 'N/A'}`);
...
async function testConnection() {
    try {
        ...
        console.log(`Base de datos actual: ${dbNameResult.rows[0].current_database}`);
        ...
        if (estudiantesCheck.rows[0].table_exists) {
            console.log('✅ Tabla "estudiantes" existe');

            // Contar registros
            const countResult = await client.query('SELECT COUNT(*) as total FROM estudiantes');
            console.log(`Registros en estudiantes: ${countResult.rows[0].total}`);
            ...
            if (parseInt(countResult.rows[0].total) > 0) {
                ...
                sampleResult.rows.forEach((row, idx) => {
                    console.log(`  ${idx + 1}. ID: ${row.id}, Nombre: ${row.nombre} ${row.apellido_paterno} ${row.apellido_materno}`);
                });
            } else {
                console.log('⚠️ La tabla estudiantes está VACÍA');
            }
        } else {
