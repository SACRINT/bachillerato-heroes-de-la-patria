...
export interface PaymentIntent {
    ...
    metadata: {
        solicitud_id: number;
        tipo_pago: 'inscripcion' | 'colegiatura' | 'servicio';
    };
}
...
export interface PaymentRecord {
    ...
    monto: number;
    moneda: string;
    metodo_pago: 'tarjeta' | 'oxxo' | 'transferencia' | 'efectivo';
...
class PaymentService {
     ...
     * Crear pago con OXXO
     */
    async createOxxoPayment(data: PaymentIntent): Promise<any> {
        try {
     ...
     */
    async createPaymentRecord(data: Partial<PaymentRecord>): Promise<any> {
     ...
     */
    async getPaymentsBySolicitud(solicitudId: number): Promise<any[]> {
     ...
     */
    async getPaymentStats(filters?: { fecha_desde?: Date; fecha_hasta?: Date }): Promise<any> {
        let query = `
            SELECT 
     ...
     */
    async checkExpiredOxxoPayments(): Promise<number> {
     ...
     * Reembolsar pago
     */
    async refundPayment(paymentId: number, motivo: string): Promise<any> {
        const payment = await executeQuery(`
            SELECT * FROM pagos_inscripcion WHERE id = $1
        `, [paymentId]) as any[];
