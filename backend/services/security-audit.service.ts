...
import SecurityAuditDAO from '../data/security-audit.dao';
...
const EVENT_TYPES = {
    ...
    ACCESS_DENIED: 'access_denied', PERMISSION_DENIED: 'permission_denied', INVALID_TOKEN: 'invalid_token',
    SESSION_HIJACK_ATTEMPT: 'session_hijack_attempt',
    DATA_ACCESS: 'data_access', DATA_EXPORT: 'data_export', DATA_MODIFY: 'data_modify', DATA_DELETE: 'data_delete',
    BULK_OPERATION: 'bulk_operation',
    ATTACK_DETECTED: 'attack_detected', RATE_LIMIT_EXCEEDED: 'rate_limit_exceeded', IP_BLOCKED: 'ip_blocked',
    ACCOUNT_LOCKED: 'account_locked', SUSPICIOUS_ACTIVITY: 'suspicious_activity',
    ADMIN_ACTION: 'admin_action', CONFIG_CHANGE: 'config_change', USER_CREATE: 'user_create', USER_DELETE: 'user_delete',
...
const SEVERITY_LEVELS = { DEBUG: 0, INFO: 1, WARNING: 2, ERROR: 3, CRITICAL: 4 } as const;
...
class SecurityAuditService {
    ...
    constructor() {
        ...
        this.flushInterval = setInterval(() => {
            this.flush().catch(err => console.error('[SECURITY-AUDIT] Error en flush:', err));
        }, this.flushIntervalMs);
        console.log('[SECURITY-AUDIT] Servicio de auditoría inicializado');
    }

    async log(eventType: string, data: LogData = {}): Promise<AuditEvent> {
    ...
    async logLogin(userId: number, ip: string, success: boolean, details: any = {}): Promise<AuditEvent> {
        return this.log(success ? this.eventTypes.LOGIN_SUCCESS : this.eventTypes.LOGIN_FAILURE, { userId, ip, success, details });
    }

    async logAccessDenied(userId: number | null, ip: string, resource: string, reason: string): Promise<AuditEvent> {
        return this.log(this.eventTypes.ACCESS_DENIED, { userId, ip, resource, success: false, details: { reason } });
    }

    async logDataAccess(userId: number, resource: string, action: string, details: any = {}): Promise<AuditEvent> {
        return this.log(this.eventTypes.DATA_ACCESS, { userId, resource, action, details });
    }

    async logAdminAction(userId: number, action: string, targetResource: string, details: any = {}): Promise<AuditEvent> {
        return this.log(this.eventTypes.ADMIN_ACTION, { userId, action, resource: targetResource, details });
    }

    async logSecurityThreat(ip: string, threatType: string, details: any = {}): Promise<AuditEvent> {
        return this.log(this.eventTypes.ATTACK_DETECTED, { ip, success: false, details: { threatType, ...details } });
    }
    ...
    determineSeverity(eventType: string): number {
        ...
        if (critical.includes(eventType as any)) return this.severityLevels.CRITICAL;
        if (error.includes(eventType as any)) return this.severityLevels.ERROR;
        if (warning.includes(eventType as any)) return this.severityLevels.WARNING;
    ...
    async query(filters: any = {}, options: any = {}): Promise<any> {
    ...
    async getSummary(startDate: Date, endDate: Date): Promise<any> { return SecurityAuditDAO.getSummary(startDate, endDate); }
    async getSuspiciousActivity(hours: number = 24): Promise<any[]> { return SecurityAuditDAO.getSuspiciousActivity(new Date(Date.now() - hours * 3600000)); }
    async getUserTimeline(userId: number, limit: number = 100): Promise<any[]> { return SecurityAuditDAO.getUserTimeline(userId, limit); }

    async exportLogs(filters: any, format: 'json' | 'csv' = 'json'): Promise<string | any[]> {
        ...
        if (format === 'csv') {
            ...
            const rows = result.logs.map((l: any) => [l.event_id, l.timestamp, l.event_type, l.severity, l.user_id || '', l.ip_address || '', l.resource || '', l.action || '', l.success].join(','));
            return headers + '\n' + rows.join('\n');
        }
        return result.logs;
    }
    ...
    async cleanup(retentionDays: number = 90): Promise<number> {
        ...
        const deleted = await SecurityAuditDAO.cleanup(cutoffDate);
        console.log(`[SECURITY-AUDIT] Eliminados ${deleted} logs antiguos`);
        return deleted;
    }

    getStats(): any {
        return { bufferSize: this.eventBuffer.length, bufferMaxSize: this.bufferMaxSize, flushIntervalMs: this.flushIntervalMs, eventTypes: Object.keys(this.eventTypes).length, severityLevels: Object.keys(this.severityLevels) };
    }

    async stop(): Promise<void> { clearInterval(this.flushInterval); await this.flush(); console.log('[SECURITY-AUDIT] Servicio detenido'); }
}
