...
const SEVERITY_LEVELS = { DEBUG: 0, INFO: 1, WARNING: 2, ERROR: 3, CRITICAL: 4 };
exports.SEVERITY_LEVELS = SEVERITY_LEVELS;
...
class SecurityAuditService {
    ...
    async flush() {
        ...
        this.eventBuffer = [];
        try {
            return await security_audit_dao_1.default.insertEventsBatch(events);
        }
        catch (error) {
            console.error('[SECURITY-AUDIT] Error persistiendo:', error);
    ...
    async query(filters = {}, options = {}) {
        ...
        const logs = await security_audit_dao_1.default.query(query, params);
        const countQuery = query.replace(/SELECT \*/, 'SELECT COUNT(*)').split('ORDER BY')[0];
        const total = await security_audit_dao_1.default.count(countQuery, params.slice(0, -2));
        return { logs, pagination: { page, limit, total, totalPages: Math.ceil(total / limit) } };
    }
    async getSummary(startDate, endDate) { return security_audit_dao_1.default.getSummary(startDate, endDate); }
    async getSuspiciousActivity(hours = 24) { return security_audit_dao_1.default.getSuspiciousActivity(new Date(Date.now() - hours * 3600000)); }
    async getUserTimeline(userId, limit = 100) { return security_audit_dao_1.default.getUserTimeline(userId, limit); }
    ...
    async cleanup(retentionDays = 90) {
        ...
        const deleted = await security_audit_dao_1.default.cleanup(cutoffDate);
