/**
 * ✅ CONSENT MANAGEMENT SERVICE - v2.0.0
...
async function grantConsent(userId, consentType, options = {}) {
  ...
  if (!Object.values(CONSENT_TYPES).find(c => c.type === consentType)) throw new Error(`Invalid consent type: ${consentType}`);
...
async function revokeConsent(userId, consentType) {
  ...
  if (consentConfig && consentConfig.required) throw new Error(`Cannot revoke required consent: ${consentType}. Deactivate account instead.`);
...
async function getUserConsents(userId) { return GDPRDAO.getUserConsents(userId); }
async function hasActiveConsent(userId, consentType) { return GDPRDAO.hasActiveConsent(userId, consentType); }
...
async function bulkGrantConsents(userId, consents, options = {}) {
  ...
  const results = [];
  for (const { type, granted } of consents) { if (granted) results.push(await grantConsent(userId, type, options)); }
...
async function getCurrentPrivacyPolicyVersion() { return GDPRDAO.getCurrentPrivacyPolicyVersion(); }
...
module.exports = { CONSENT_TYPES, grantConsent, revokeConsent, getUserConsents, hasActiveConsent, getMissingRequiredConsents, bulkGrantConsents, createPrivacyPolicyVersion, getCurrentPrivacyPolicyVersion, generateConsentReport };
