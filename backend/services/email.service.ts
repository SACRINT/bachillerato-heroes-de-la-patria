...
export class EmailService {
    ...
    private templatesCache: { [key: string]: any };
    private from: string;
    ...
    registerHandlebarsHelpers(): void {
        ...
        this._handlebars.registerHelper('formatDate', function (date: string) {
            if (!date) return '';
        ...
        this._handlebars.registerHelper('formatDateTime', function (date: string) {
            if (!date) return '';
            const d = new Date(date);
        ...
        // Helper condicional
        this._handlebars.registerHelper('ifEquals', function (arg1: any, arg2: any, options: any) {
            return (arg1 == arg2) ? options.fn(this) : options.inverse(this);
        });

        // Helper para URLs absolutas
        this._handlebars.registerHelper('absoluteUrl', function (path: string) {
     ...
     */
    async sendEmail({ to, subject, template, data, attachments = [] }: EmailOptions): Promise<any> {
        if (!this.initialized) {
            await this.init();
        }
        ...
        try {
            ...
            if (previewUrl) {
                devLogger.log(`🔗 Vista previa: ${previewUrl}`);
            }
     ...
     */
    async sendWelcomeEmail(user: any): Promise<any> {
     ...
     */
    async sendEventNotification(user: any, event: any): Promise<any> {
        return await this.sendEmail({
            to: user.email,
     ...
     * Enviar newsletter
     */
    async sendNewsletter(user: any, newsletter: any): Promise<any> {
        return await this.sendEmail({
            to: user.email,
     ...
     */
    async sendPasswordRecovery(user: any, resetToken: string): Promise<any> {
     ...
     */
    async sendInscriptionConfirmation(user: any, activity: any): Promise<any> {
        return await this.sendEmail({
            to: user.email,
     ...
     */
    async sendNewsNotification(user: any, noticia: any): Promise<any> {
        return await this.sendEmail({
            to: user.email,
     ...
     */
    async sendBulkEmails(emails: EmailOptions[], delayMs: number = 100): Promise<any> {
        const results = [];
