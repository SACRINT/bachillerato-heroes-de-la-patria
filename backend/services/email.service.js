...
class EmailService {
    ...
    constructor(options = {}) {
        ...
        this._nodemailer = options.nodemailerModule || nodemailer_1.default;
        this._fs = options.fsModule || promises_1.default;
        this._handlebars = options.handlebarsModule || handlebars_1.default;
    ...
    async init() {
        ...
        try {
            ...
            if (process.env.SMTP_HOST && process.env.SMTP_USER) {
                ...
                devLogger_1.default.log(`📧 Email Service usando SMTP Real: ${process.env.SMTP_HOST}`);
            }
            // Prioridad 2: Fallback a Ethereal (solo si no hay SMTP)
                ...
                devLogger_1.default.log('📧 Email Service en modo desarrollo (Ethereal Email)');
                devLogger_1.default.log(`👤 Usuario: ${testAccount.user}`);
            }
            // Verificar conexión
            await this.transporter.verify();
            this.initialized = true;
            devLogger_1.default.log('✅ Email Service inicializado correctamente');
            // Registrar helpers de Handlebars
            this.registerHandlebarsHelpers();
        }
        catch (error) {
            devLogger_1.default.error('❌ Error al inicializar Email Service:', error);
            throw error;
        }
    }
    /**
    ...
    registerHandlebarsHelpers() {
        ...
        devLogger_1.default.log('✅ Handlebars helpers registrados');
    }
    /**
    ...
    async loadTemplate(templateName) {
        ...
        try {
            ...
            devLogger_1.default.error(`❌ Error al cargar plantilla ${templateName}:`, error);
    ...
    async sendEmail({ to, subject, template, data, attachments = [] }) {
        ...
        try {
            ...
            devLogger_1.default.log(`✅ Email enviado a ${to}: ${info.messageId}`);
            // En desarrollo, mostrar URL de previsualización
            const previewUrl = process.env.NODE_ENV !== 'production' ? this._nodemailer.getTestMessageUrl(info) : null;
            if (previewUrl) {
                devLogger_1.default.log(`🔗 Vista previa: ${previewUrl}`);
            }
            return {
                success: true,
            ...
            devLogger_1.default.error(`❌ Error al enviar email a ${to}:`, error);
            throw error;
        }
    }
    /**
    ...
    async sendBulkEmails(emails, delayMs = 100) {
        ...
        devLogger_1.default.log(`📊 Envío en lote completado: ${successful} exitosos, ${failed} fallidos`);
        return {
            total: emails.length,
            successful,
            failed,
    ...
    clearTemplateCache() {
        this.templatesCache = {};
        devLogger_1.default.log('🗑️ Caché de plantillas limpiado');
    }
}
...
module.exports.EmailService = EmailService; // Mantener compatibilidad con require
