...
// ============================================
// CHALLENGES SERVICE (Retos Diarios)
// ============================================
...
export interface Challenge {
    ...
    descripcion: string;
    objetivo: number;
    metrica: string; // tareas_completadas, logins, calificacion_minima, etc.
...
class ChallengesService {
    ...
    async generateDailyChallenges(): Promise<Challenge[]> {
        ...
        for (const template of selected) {
            ...
            template.metrica, template.coins, template.xp]) as any[];
     ...
     */
    async updateProgress(userId: number, metrica: string, incremento: number = 1): Promise<void> {
     ...
     */
    async claimReward(userId: number, challengeId: number): Promise<any> {
...
class StreaksService {
     ...
     */
    async recordDailyLogin(userId: number): Promise<any> {
        const today = new Date().toISOString().split('T')[0];

        // Verificar si ya hizo login hoy
        const todayLogin = await executeQuery(`
        ...
        const streak = await executeQuery(`
        ...
        const result = await executeQuery(`
            ...
            RETURNING *
        `, [userId, newStreak, maxStreak]) as any[];
    ...
    private async checkStreakMilestones(userId: number, streak: number): Promise<void> {
...
export interface Achievement {
    ...
    descripcion: string;
    icono: string;
    categoria: 'academico' | 'social' | 'tecnico' | 'especial';
    rareza: 'comun' | 'raro' | 'epico' | 'legendario';
    criterio: any; // JSON con condiciones
    recompensa_coins: number;
...
class AchievementsService {
    ...
    private async meetsRequirements(userId: number, criterio: any): Promise<boolean> {
...
class LeaderboardService {
     ...
     */
    async getSchoolLeaderboard(tenantId: number, timeframe: 'semanal' | 'mensual' | 'global' = 'mensual', limit: number = 100): Promise<any[]> {
        let dateFilter = '';
        if (timeframe === 'semanal') dateFilter = "AND created_at >= CURRENT_DATE - INTERVAL '7 days'";
        else if (timeframe === 'mensual') dateFilter = "AND created_at >= CURRENT_DATE - INTERVAL '30 days'";
     ...
     */
    async getUserRank(userId: number, tenantId: number): Promise<any> {
        const result = await executeQuery(`
            WITH ranked AS (
                SELECT 
                    id,
...
class CompetitionsService {
    ...
    async createCompetition(data: {
        ...
        data.fecha_inicio, data.fecha_fin, data.premio_ganador]) as any[];
     ...
     */
    async getCompetitionResults(competitionId: number): Promise<any[]> {
        return await executeQuery(`
            SELECT 
                cp.grupo_id,
     ...
     */
    async finalizeCompetition(competitionId: number): Promise<void> {
