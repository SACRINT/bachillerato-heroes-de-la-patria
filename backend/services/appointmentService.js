...
class AppointmentService {
    ...
    async getAppointment(id) {
        try {
            ...
            if (!appointment)
                throw new ServiceError('Cita no encontrada', 404);
            return appointment;
        }
        catch (error) {
    ...
    async confirmAppointment(idOrToken, isToken = false) {
        try {
            ...
            if (appointment.estado !== 'pendiente')
                throw new ServiceError('La cita no está en estado pendiente', 400);
    ...
    async deleteAppointment(id) {
        try {
            ...
            if (!deleted)
                throw new ServiceError('No se pudo eliminar la cita', 500);
    ...
    _validateAppointmentData(data) {
        if (!data.nombre_completo)
            throw new ServiceError('Nombre completo requerido', 400);
        if (!data.email)
            throw new ServiceError('Email requerido', 400);
        if (!data.fecha_solicitada)
            throw new ServiceError('Fecha requerida', 400);
        if (!data.hora_solicitada)
            throw new ServiceError('Hora requerida', 400);
        if (!data.motivo)
