...
class StripePaymentsService {
     ...
     * Registrar intento de pago en BD
     */
    async createPaymentIntent(userId, sessionId, pkg, totalIacoins) {
     ...
     */
    async processCompletedPayment(sessionId) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            // Obtener payment intent
            const intentResult = await client.query('SELECT * FROM payment_intents WHERE stripe_session_id = $1 FOR UPDATE', [sessionId]);
            ...
            await client.query(`UPDATE payment_intents SET status = 'completed', completed_at = NOW() WHERE id = $1`, [intent.id]);
            ...
            const walletResult = await client.query(`UPDATE wallet 
                 ...
                 WHERE user_id = $2
                 RETURNING balance`, [intent.iacoins_amount, intent.user_id]);
            let newBalance;
            if (walletResult.rows.length === 0) {
                // Crear wallet si no existe
                const createResult = await client.query(`INSERT INTO wallet (user_id, balance, total_earned, total_spent, total_purchased)
                     VALUES ($1, $2, 0, 0, $2)
                     RETURNING balance`, [intent.user_id, intent.iacoins_amount]);
    ...
    async verifySession(sessionId) {
        const result = await database_1.pool.query('SELECT * FROM payment_intents WHERE stripe_session_id = $1', [sessionId]);
        return result.rows[0] || null;
    }
    /**
    ...
    async handleWebhook(event) {
        switch (event.type) {
            ...
            case 'payment_intent.payment_failed':
                ...
                await database_1.pool.query(`UPDATE payment_intents SET status = 'failed', error_message = $1 
                     WHERE stripe_payment_intent = $2`, [failedIntent.last_payment_error?.message || 'Payment failed', failedIntent.id]);
                break;
            default:
    ...
    async getUserPurchaseHistory(userId, limit = 20) {
        const result = await database_1.pool.query(`SELECT * FROM payment_intents 
             WHERE user_id = $1 
...
exports.default = stripePaymentsService;
