...
export interface GradeValidationRequest {
...
export interface GradeAuditEntry {
    calificacion_id: number;
    usuario_id: number;
    accion: 'creacion' | 'modificacion' | 'validacion' | 'rechazo';
...
export interface RiskAlert {
    ...
    tipo_alerta: 'bajo_promedio' | 'reprobado' | 'ausentismo' | 'irregular';
...
class GradesValidationService {
     ...
     */
    async calculateStudentAverage(studentId: number, periodo?: string): Promise<number> {
        const query = periodo
        ...
        return result[0]?.promedio ? parseFloat(result[0].promedio) : 0;
    }

    /**
     * Calcular promedio por materia
     */
    async calculateSubjectAverage(studentId: number, materiaId: number): Promise<number> {
        const result = await executeQuery(`
            SELECT AVG(calificacion) as promedio
            FROM calificaciones
            WHERE estudiante_id = $1 AND materia_id = $2 AND status != 'rechazado'
        `, [studentId, materiaId]) as any[];
     ...
     */
    async validateGrade(validationData: GradeValidationRequest): Promise<void> {
     ...
     */
    async logAudit(entry: GradeAuditEntry): Promise<void> {
        await executeQuery(`
     ...
     */
    async getAuditHistory(calificacionId: number): Promise<any[]> {
        ...
        `, [calificacionId]) as any[];
    }

    /**
     * Verificar y crear alertas de estudiantes en riesgo
     */
    async checkAndCreateRiskAlerts(studentId: number): Promise<void> {
        ...
        const ausentismos = await executeQuery(`
        ...
        const totalFaltas = parseInt(ausentismos[0]?.total || 0);
     ...
     */
    async createRiskAlert(alert: RiskAlert): Promise<void> {
        ...
        `, [alert.estudiante_id, alert.tipo_alerta]) as any[];
     ...
     */
    async getActiveAlerts(filters?: { estudiante_id?: number; severidad?: string }): Promise<any[]> {
        let query = `
            SELECT 
                a.*,
    ...
    private async notifyRiskAlert(alert: RiskAlert): Promise<void> {
        ...
        const student = await executeQuery(`
            ...
            WHERE e.id = $1
        `, [alert.estudiante_id]) as any[];

        if (!student || student.length === 0) return;

        // Obtener tutores/padres
        const tutores = await executeQuery(`
     ...
     */
    async getPendingValidations(coordinadorId?: number): Promise<any[]> {
        return await executeQuery(`
            SELECT 
                c.*,
                e.nombre || ' ' || e.apellido_paterno as estudiante_nombre,
     ...
     */
    async generatePeriodReport(periodo: string, grupoId?: number): Promise<any> {
        const query = grupoId
            ? `SELECT 
        ...
        const grades = await executeQuery(query, params) as any[];
...
export default new GradesValidationService();
