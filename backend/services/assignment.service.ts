...
export interface Assignment {
    ...
    titulo: string;
    descripcion: string;
    tipo: 'tarea' | 'proyecto' | 'investigacion' | 'practica' | 'examen';
    ...
    status: 'borrador' | 'publicado' | 'cerrado' | 'cancelado';
}
...
export interface Submission {
    ...
    status: 'pendiente' | 'entregado' | 'revisado' | 'calificado';
}
...
class AssignmentService {
     ...
     * Crear nueva tarea
     */
    async createAssignment(data: Assignment): Promise<any> {
     ...
     */
    async assignToStudents(tareaId: number, materiaId: number): Promise<void> {
        await executeQuery(`
     ...
     */
    async getTeacherAssignments(docenteId: number, filters?: {
        materia_id?: number;
        tipo?: string;
        status?: string;
    }): Promise<any[]> {
     ...
     */
    async getAssignmentSubmissions(tareaId: number): Promise<any[]> {
     ...
     * Calificar entrega
     */
    async gradeSubmission(entregaId: number, calificacion: number, retroalimentacion?: string): Promise<any> {
        const result = await executeQuery(`
            ...
            WHERE id = $3
            RETURNING *
        `, [calificacion, retroalimentacion, entregaId]) as any[];
     ...
     */
    async bulkGrade(entregaIds: number[], calificacion: number, retroalimentacion?: string): Promise<number> {
        const result = await executeQuery(`
            UPDATE entregas_tareas
            SET 
                calificacion = $1,
     ...
     * Actualizar tarea
     */
    async updateAssignment(id: number, data: Partial<Assignment>): Promise<any> {
        ...
        for (const field of allowedFields) {
            if (data[field as keyof Assignment] !== undefined) {
                updates.push(`${field} = $${paramIndex}`);

                // JSON fields
                if (['criterios_evaluacion', 'archivos_adjuntos'].includes(field)) {
                    params.push(JSON.stringify(data[field as keyof Assignment]));
                } else {
                    params.push(data[field as keyof Assignment]);
                }
                paramIndex++;
            }
        }
     ...
     */
    async publishAssignment(id: number): Promise<any> {
        const tarea = await executeQuery(`
     ...
     */
    async closeAssignment(id: number): Promise<any> {
     ...
     */
    async getAssignmentStats(docenteId: number): Promise<any> {
     ...
     */
    async getUpcomingDeadlines(docenteId: number, dias: number = 7): Promise<any[]> {
        return await executeQuery(`
            SELECT 
                t.*,
                m.nombre as materia_nombre,
     ...
     */
    async sendReminders(tareaId: number): Promise<number> {
