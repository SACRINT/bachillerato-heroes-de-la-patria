...
class ForumDAO {
    ...
    static async getCategories(userRole = 'estudiante') {
        ...
        `;
        return (0, database_1.executeQuery)(query, [userRole]);
    }
    static async getCategoryBySlug(slug) {
        const query = `SELECT * FROM forum_categories WHERE slug = $1 AND is_active = true`;
        const results = await (0, database_1.executeQuery)(query, [slug]);
    ...
    static async getTopics(options = {}) {
        const { categoryId, authorId, status = 'open', topicType, search, sortBy = 'last_reply_at', sortOrder = 'DESC', limit = 20, offset = 0 } = options;
        let query = `
    ...
    static async updateTopic(topicId, authorId, fields, values) {
        ...
        `;
        const results = await (0, database_1.executeQuery)(query, values);
        return results[0];
    }
    static async incrementViewCount(topicId) {
        await (0, database_1.executeQuery)(`UPDATE forum_topics SET view_count = view_count + 1 WHERE id = $1`, [topicId]);
    }
    static async getTopicAuthor(topicId) {
        const result = await (0, database_1.executeQuery)(`SELECT author_id FROM forum_topics WHERE id = $1`, [topicId]);
    ...
    static async createPost(topicId, parentPostId, content, authorId) {
        ...
        const results = await (0, database_1.executeQuery)(query, [topicId, parentPostId, content, authorId]);
        return results[0];
    }
    static async updatePost(postId, authorId, content) {
        ...
        `;
        const results = await (0, database_1.executeQuery)(query, [content, authorId, postId]);
        return results[0];
    }
    static async deletePost(postId, userId, reason) {
        const query = `
            ...
            RETURNING *
        `;
        const results = await (0, database_1.executeQuery)(query, [userId, reason, postId]);
        return results.length > 0;
    }
    static async getPostAuthor(postId) {
        const result = await (0, database_1.executeQuery)(`SELECT author_id FROM forum_posts WHERE id = $1`, [postId]);
        return result[0]?.author_id;
    }
    static async clearTopicSolutions(topicId) {
        await (0, database_1.executeQuery)(`UPDATE forum_posts SET is_solution = false WHERE topic_id = $1`, [topicId]);
    }
    static async markPostAsSolution(postId) {
        const result = await (0, database_1.executeQuery)(`UPDATE forum_posts SET is_solution = true WHERE id = $1 RETURNING author_id`, [postId]);
        return result[0];
    }
    static async updateTopicSolution(topicId, postId) {
        await (0, database_1.executeQuery)(`UPDATE forum_topics SET solution_post_id = $1, solved_at = NOW(), status = 'solved' WHERE id = $2`, [postId, topicId]);
    }
    // ==========================================
    // REACCIONES
    // ==========================================
    static async getTopicReaction(userId, topicId) {
        const results = await (0, database_1.executeQuery)(`SELECT id, reaction_type FROM forum_reactions WHERE user_id = $1 AND topic_id = $2`, [userId, topicId]);
        return results[0];
    }
    static async getPostReaction(userId, postId) {
        const results = await (0, database_1.executeQuery)(`SELECT id, reaction_type FROM forum_reactions WHERE user_id = $1 AND post_id = $2`, [userId, postId]);
        return results[0];
    }
    static async deleteReaction(reactionId) {
        await (0, database_1.executeQuery)(`DELETE FROM forum_reactions WHERE id = $1`, [reactionId]);
    }
    static async updateReaction(reactionId, reactionType) {
        await (0, database_1.executeQuery)(`UPDATE forum_reactions SET reaction_type = $1 WHERE id = $2`, [reactionType, reactionId]);
    }
    static async createTopicReaction(userId, topicId, reactionType) {
        await (0, database_1.executeQuery)(`INSERT INTO forum_reactions (user_id, topic_id, reaction_type) VALUES ($1, $2, $3)`, [userId, topicId, reactionType]);
    }
    ...
    static async subscribeTopic(userId, topicId) {
        ...
        `;
        const results = await (0, database_1.executeQuery)(query, [userId, topicId]);
        return results[0];
    }
    static async unsubscribeTopic(userId, topicId) {
        await (0, database_1.executeQuery)(`DELETE FROM forum_subscriptions WHERE user_id = $1 AND topic_id = $2`, [userId, topicId]);
    }
    ...
    static async findUserByUsername(username) {
        const results = await (0, database_1.executeQuery)(`SELECT id FROM usuarios WHERE username = $1 OR email LIKE $2 LIMIT 1`, [username, `${username}@%`]);
        return results[0];
    }
    static async createMention(postId, mentionedUserId) {
        await (0, database_1.executeQuery)(`INSERT INTO forum_mentions (post_id, mentioned_user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [postId, mentionedUserId]);
    }
    ...
    static async markMentionsAsRead(userId, mentionIds = null) {
        if (mentionIds && mentionIds.length > 0) {
            await (0, database_1.executeQuery)(`UPDATE forum_mentions SET is_read = true, read_at = NOW() WHERE mentioned_user_id = $1 AND id = ANY($2)`, [userId, mentionIds]);
        }
        else {
            await (0, database_1.executeQuery)(`UPDATE forum_mentions SET is_read = true, read_at = NOW() WHERE mentioned_user_id = $1 AND is_read = false`, [userId]);
        }
    }
    // ==========================================
    // REPORTES
    ...
    static async createReport(userId, topicId, postId, reason, description) {
        ...
        `;
        const results = await (0, database_1.executeQuery)(query, [userId, topicId, postId, reason, description]);
        return results[0];
    }
    ...
    static async createPoll(topicId, question, options, allowsMultiple, endsAt) {
        ...
        const results = await (0, database_1.executeQuery)(query, [topicId, question, JSON.stringify(options), allowsMultiple, endsAt]);
        return results[0];
    }
    static async updateTopicType(topicId, type) {
        await (0, database_1.executeQuery)(`UPDATE forum_topics SET topic_type = $1 WHERE id = $2`, [type, topicId]);
    }
    static async getPollVote(pollId, userId) {
        const results = await (0, database_1.executeQuery)(`SELECT id FROM forum_poll_votes WHERE poll_id = $1 AND user_id = $2`, [pollId, userId]);
        return results[0];
    }
    static async createPollVote(pollId, userId, optionIds) {
        await (0, database_1.executeQuery)(`INSERT INTO forum_poll_votes (poll_id, user_id, option_ids) VALUES ($1, $2, $3)`, [pollId, userId, optionIds]);
    }
    static async getPollOptions(pollId) {
        const results = await (0, database_1.executeQuery)(`SELECT options FROM forum_polls WHERE id = $1`, [pollId]);
        return results[0]?.options;
    }
    static async updatePollOptions(pollId, options, voteCount) {
        await (0, database_1.executeQuery)(`UPDATE forum_polls SET options = $1, total_votes = total_votes + $2, voter_count = voter_count + 1 WHERE id = $3`, [JSON.stringify(options), voteCount, pollId]);
    }
    // ==========================================
    ...
    static async updateReputationLevel(userId, level) {
        await (0, database_1.executeQuery)(`UPDATE forum_user_stats SET reputation_level = $1 WHERE user_id = $2`, [level, userId]);
    }
...
exports.default = ForumDAO;
module.exports = ForumDAO;
//# sourceMappingURL=forum.dao.js.map