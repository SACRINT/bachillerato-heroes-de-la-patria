...
class ErasureDAO {
    ...
    static async getActiveLegalCases(userId: number): Promise<number> {
        const result = await pool.query('SELECT COUNT(*) FROM legal_cases WHERE user_id = $1 AND status = \'active\'', [userId]);
        return parseInt(result.rows[0].count);
    }

    static async getPublicContent(userId: number): Promise<number> {
        const result = await pool.query('SELECT COUNT(*) FROM forum_posts WHERE user_id = $1 AND is_public = true', [userId]);
        return parseInt(result.rows[0].count);
    }

    static async pseudonymizeUser(client: any, userId: number, pseudonym: string): Promise<void> {
    ...
    static async deleteNonEssentialData(client: any, userId: number): Promise<void> {
    ...
    static async anonymizePublicContent(client: any, userId: number, pseudonym: string): Promise<void> {
    ...
    static async logErasureAction(client: any, userId: number, requestedBy: number, reason: string, validation: any): Promise<void> {
    ...
    static async getErasureLog(userId: number): Promise<ErasureLog | null> {
    ...
    static async restoreUser(userId: number, originalData: UserOriginalData): Promise<void> {
    ...
    static getConnection(): Promise<any> { return pool.connect(); }

    /**
     * Crear solicitud de eliminación DSAR
     */
    static async createDsarErasureRequest(requestId: string, userId: number, email: string, metadata: any): Promise<void> {
        await pool.query(
     ...
     */
    static async logErasureRestoration(adminId: number, userId: string, reason: string): Promise<void> {
        await pool.query(
     ...
     */
    static async getPendingErasureRequests(): Promise<DsarRequest[]> {
