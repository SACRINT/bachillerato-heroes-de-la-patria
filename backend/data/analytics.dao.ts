...
export interface PopularItem {
    ...
    categoria?: string;
    score?: number; // Calculated score for Alt method
    item_id?: number; // Alias for Alt method
    nombre?: string; // Alias for Alt method
}

export interface RecommendationStats {
    total_interactions: number;
    by_type: Array<{ interaction_type: string; count: number }>;
    top_items: Array<{ item_type: string; item_id: number; interactions: number }>;
    top_users: Array<{ user_id: number; interactions: number }>;
}

export interface InteractionAnalytics extends RecommendationStats {
    interactions_by_type: Array<{ interaction_type: string; count: number }>; // Alias for by_type in getInteractionAnalytics
}
...
export interface TimeSeriesPoint {
    date: Date | string;
    value: number;
    ds?: Date | string; // Alias for date in some charts
    y?: number; // Alias for value in some charts
}
...
export interface SystemHealth {
    top_tables: Array<{ table_name: string; row_count: number }>;
...
class AnalyticsDAO {
    ...
    static async getPopularItems(type: string, limit: number = 10): Promise<PopularItem[]> {
    ...
    static async getSimilarItems(type: string, itemId: number, limit: number = 5): Promise<SimilarItem[]> {
        const tableMap: Record<string, string> = {
            'courses': 'cursos',
            'materials': 'materiales',
            'activities': 'actividades',
    ...
    static async getRecommendationsHealth(): Promise<RecommendationHealth> {
    ...
    static async getPopularItemsAlt(type: string, limit: number = 10): Promise<PopularItem[]> {
        const tableMap: Record<string, string> = {
    ...
    static async getSimilarItemsAlt(type: string, itemId: number, limit: number = 5): Promise<SimilarItem[]> {
        const tableMap: Record<string, string> = {
            'courses': 'cursos_disponibles',
            'materials': 'materiales_estudio',
            'activities': 'actividades_extra',
    ...
    static async getInteractionAnalytics(filters: { dateFrom?: string | Date; dateTo?: string | Date } = {}): Promise<InteractionAnalytics> {
        let whereClause = '1=1';
    ...
    static async getHistoricalGrades(studentId: string): Promise<TimeSeriesPoint[]> {
        const query = `
    ...
    static async getHistoricalEnrollments(): Promise<TimeSeriesPoint[]> {
        const query = `
    ...
    static async getHistoricalDropout(): Promise<TimeSeriesPoint[]> {
        const query = `
    ...
    static async getGradesTrend(startDate: string | Date = '2024-01-01', endDate: string | Date = '2025-12-31'): Promise<TimeSeriesPoint[]> {
        const query = `
            SELECT
    ...
    static async getPredictiveSummary(): Promise<PredictiveSummary> {
        const result = await pool.query(`
    ...
    static async getStudentFeatures(studentId: string): Promise<StudentFeatures | null> {
        const query = `
    ...
    static async getActiveStudents(limit: number = 100): Promise<ActiveStudent[]> {
        const result = await pool.query(`
    ...
    static async getTableStats(tableName: string | null = null): Promise<TableStats[]> {
        let query = `
            SELECT
    ...
    static async getIndexUsage(tableName: string | null = null, unusedOnly: boolean = false): Promise<IndexUsage[]> {
        let query = `
            SELECT
                schemaname,
                relname as table_name,
    ...
    static async executeMaintenanceCommand(command: string): Promise<void> {
