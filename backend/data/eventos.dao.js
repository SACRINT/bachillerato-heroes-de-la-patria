...
class EventosDAO {
    static async slugExists(slug) {
        const result = await (0, database_1.executeQuery)('SELECT id FROM eventos WHERE slug = $1', [slug]);
    ...
    static async getAll(options) {
        ...
        if (categoria) {
            pc++;
            query += ` AND categoria = $${pc}`;
            params.push(categoria);
        }
        if (modalidad) {
            pc++;
            query += ` AND modalidad = $${pc}`;
            params.push(modalidad);
        }
        if (destacado !== undefined) {
            pc++;
            query += ` AND destacado = $${pc}`;
        ...
        if (categoria) {
            cpc++;
            cq += ` AND categoria = $${cpc}`;
            cp.push(categoria);
        }
        if (modalidad) {
            cpc++;
            cq += ` AND modalidad = $${cpc}`;
            cp.push(modalidad);
        }
        if (destacado !== undefined) {
            cpc++;
            cq += ` AND destacado = $${cpc}`;
            cp.push(destacado === 'true' || destacado === true);
        }
    ...
    static async getById(id) {
        const result = await (0, database_1.executeQuery)('SELECT * FROM eventos WHERE id = $1', [id]);
        return result[0] || null;
    }
    static async getBySlug(slug) {
        const result = await (0, database_1.executeQuery)('SELECT * FROM eventos WHERE slug = $1', [slug]);
        return result[0] || null;
    }
    ...
    static async getCalendarEvents(options) {
        ...
        let pc = 1;
        if (start) {
            query += ` AND fecha_inicio >= $${pc}`;
            params.push(start);
            pc++;
        }
        if (end) {
            query += ` AND fecha_fin <= $${pc}`;
            params.push(end);
            pc++;
        }
...
exports.default = EventosDAO;
module.exports = EventosDAO;
//# sourceMappingURL=eventos.dao.js.map