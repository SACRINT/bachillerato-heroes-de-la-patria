...
class EgresadosDAO {
    ...
    static async getAprobados(limit: number = 50, offset: number = 0): Promise<EgresadoRow[]> {
        const query = `
    ...
    static async getStats(): Promise<EgresadoStats> {
        const query = `
    ...
    static async createPendingConfirmation(
        email: string,
    ...
    static async getPendingByToken(token: string): Promise<PendingConfirmation | null> {
        const result = await executeQuery(
            'SELECT * FROM egresados_pending_confirmation WHERE confirmation_token = $1',
            [token]
        );
        return result[0] || null;
    }

    static async confirmEmail(token: string): Promise<ConfirmEmailResult> {
        const client: PoolClient = await pool.connect();
        try {
            ...
            try {
                ...
                if (existing.rows.length > 0) {
                    await client.query(
                        'UPDATE pendientes_aprobacion SET datos_json = $1, fecha_solicitud = NOW(), email_confirmado = $2, estado = $3 WHERE id = $4',
                        [JSON.stringify(datosJSON), true, 'pendiente', existing.rows[0].id]
                    );
                } else {
                    await client.query(
                        'INSERT INTO pendientes_aprobacion (tipo_solicitud, email_usuario, datos_json, estado, email_confirmado, fecha_solicitud) VALUES ($1, $2, $3, $4, $5, NOW())',
