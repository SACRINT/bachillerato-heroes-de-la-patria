...
class ComunicadosDAO {
    ...
    static async slugExists(slug: string): Promise<boolean> {
        const result = await executeQuery('SELECT id FROM comunicados WHERE slug = $1', [slug]);
        return result.length > 0;
    }

    static async create(data: ComunicadoCreateData): Promise<ComunicadoRow> {
        const {
    ...
    static async getAll(filters: ComunicadoFilters): Promise<{ comunicados: ComunicadoRow[]; total: number }> {
        ...
        if (estado) { paramCount++; query += ` AND estado = $${paramCount}`; params.push(estado); }
        if (categoria) { paramCount++; query += ` AND categoria = $${paramCount}`; params.push(categoria); }
        if (destacada !== undefined) { paramCount++; query += ` AND destacada = $${paramCount}`; params.push(destacada === 'true' || destacada === true); }

        query += ` ORDER BY fecha_creacion DESC LIMIT $${paramCount + 1} OFFSET $${paramCount + 2}`;
        params.push(limit, offset);
        const comunicados = await executeQuery(query, params);

        let countQuery = 'SELECT COUNT(*) FROM comunicados WHERE 1=1';
        const countParams: (string | boolean)[] = [];
        let cp = 0;
        if (estado) { cp++; countQuery += ` AND estado = $${cp}`; countParams.push(estado); }
        if (categoria) { cp++; countQuery += ` AND categoria = $${cp}`; countParams.push(categoria); }
        if (destacada !== undefined) { cp++; countQuery += ` AND destacada = $${cp}`; countParams.push(destacada === 'true' || destacada === true); }

        const countResult = await executeQuery(countQuery, countParams);
        return { comunicados, total: parseInt(countResult[0].count) };
    }

    static async getStats(): Promise<{ total: number; publicadas: number; borradores: number; destacadas: number; vistas_totales: number }> {
        const query = `
    ...
    static async getById(id: number): Promise<ComunicadoRow | null> {
        const result = await executeQuery('SELECT * FROM comunicados WHERE id = $1', [id]);
        return result[0] || null;
    }

    static async getBySlug(slug: string): Promise<ComunicadoRow | null> {
        const result = await executeQuery('SELECT * FROM comunicados WHERE slug = $1', [slug]);
        return result[0] || null;
    }
    ...
    static async update(id: number, data: Partial<ComunicadoCreateData>): Promise<ComunicadoRow | null> {
    ...
    static async archive(id: number): Promise<{ id: number; titulo: string } | null> {
