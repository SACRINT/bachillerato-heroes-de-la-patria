...
class AuditDAO {
    ...
    static async log(userId, action, entity, entityId, oldData, newData, ipAddress, userAgent, metadata) {
        try {
            ...
            `, [userId, action, entity, entityId, oldData ? JSON.stringify(oldData) : null, newData ? JSON.stringify(newData) : null, ipAddress, userAgent, metadata ? JSON.stringify(metadata) : null]);
            return result.rows[0].id;
        } catch { return null; }
    }

    static async getByUser(userId, action, startDate, endDate, limit, offset) {
        let query = 'SELECT * FROM audit_logs WHERE user_id = $1'; const params = [userId]; let idx = 2;
    ...
    static async getByEntity(entity, entityId, limit, offset) {
        const result = await pool.query('SELECT * FROM audit_logs WHERE entity = $1 AND entity_id = $2 ORDER BY created_at DESC LIMIT $3 OFFSET $4', [entity, entityId, limit, offset]);
        return result.rows;
    }
