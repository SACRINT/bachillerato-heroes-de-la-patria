/**
 * 📝 INSCRIPTIONS DAO @author Gemini Code @date 2025-12-05
 */
const { executeQuery, pool } = require('../config/database');

class InscriptionsDAO {
    static async checkExisting(email, activityId) {
        const result = await executeQuery('SELECT id, status FROM inscripciones_actividades WHERE student_email = $1 AND activity_id = $2', [email, activityId]);
    ...
    static async updateResubmit(id, data) {
        ...
        const result = await executeQuery(query, [data.studentName, data.studentId || 'Externo', data.studentGroup || 'No especificado', data.emergencyContact || 'No proporcionado', data.additionalInfo || '', data.ip_address, data.user_agent, id]);
        return result[0];
    }
    ...
    static async create(data) {
        ...
        const result = await executeQuery(query, [data.activityId, data.activityName, data.studentId || 'Externo', data.studentName, data.studentEmail, data.studentGroup || 'No especificado', data.emergencyContact || 'No proporcionado', data.additionalInfo || '', data.ip_address, data.user_agent]);
        return result[0];
    }

    static async getAll({ status, activity_id, student_email, limit = 50, offset = 0 }) {
