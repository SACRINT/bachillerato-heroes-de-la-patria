...
class TeacherDAO {
    ...
    static async get(id: number): Promise<TeacherRow | null> {
        const query = `
            SELECT
                d.*,
    ...
    static async getByEmail(email: string): Promise<TeacherRow | null> {
        const query = `
    ...
    static async getByEmployeeNumber(numero_empleado: string): Promise<TeacherRow | null> {
        const query = `SELECT * FROM docentes WHERE numero_empleado = $1`;
        const result = await executeQuery(query, [numero_empleado]);
        return result[0] || null;
    }

    static async list(filters: TeacherFilters = {}, limit: number = 20, offset: number = 0): Promise<TeacherRow[]> {
        let query = `
            SELECT 
    ...
    static async count(filters: TeacherFilters = {}): Promise<number> {
        let query = `
    ...
    static async create(userId: number, data: TeacherCreateData): Promise<TeacherRow> {
        const query = `
    ...
    static async update(id: number, data: TeacherUpdateData): Promise<TeacherRow | null> {
        const fields: string[] = [];
        const values: (string | number | boolean | null)[] = [];
        let paramCount = 1;

        const allowedFields: (keyof TeacherUpdateData)[] = [
    ...
    static async getSubjects(teacherId: number): Promise<TeacherSubject[]> {
        const query = `
            SELECT
    ...
    static async getSchedule(teacherId: number): Promise<TeacherSubject[]> {
        const query = `
            SELECT
    ...
    static async getSpecialties(): Promise<SpecialtyStats[]> {
        const query = `
            SELECT
                especialidad,
    ...
    static async getPublicDirectory(especialidadFilter: string | null = null): Promise<PublicTeacher[]> {
        let query = `
            SELECT 
