...
class AttendanceDAO {
    ...
    static async getByClass(classId, date) {
        ...
        `;
        const result = await (0, database_1.executeQuery)(query, [classId, date]);
        return result;
    }
    static async getMonthlyReport(studentId, year, month) {
        ...
        `;
        const result = await (0, database_1.executeQuery)(query, [studentId, year, month]);
        return result;
    }
    static async getAttendanceRate(studentId, startDate, endDate) {
        const query = `
            SELECT
        ...
        `;
        const result = await (0, database_1.executeQuery)(query, [studentId, startDate, endDate]);
        return result[0] || {
            total_registros: 0,
            asistencias: 0,
            faltas: 0,
    ...
    static async getSummaryByStudent(studentId) {
        ...
        `;
        const result = await (0, database_1.executeQuery)(query, [studentId, currentMonth, currentYear]);
        return result[0] || {
            total_registros: 0,
            asistencias: 0,
            faltas: 0,
    ...
    static async detectAbsenteeismPatterns(studentId, days = 30) {
        const query = `
            SELECT
                DATE(fecha) as fecha,
                presente,
...
exports.default = AttendanceDAO;
module.exports = AttendanceDAO;
//# sourceMappingURL=attendance.dao.js.map