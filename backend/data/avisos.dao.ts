...
class AvisosDAO {
    ...
    static async getAll(filters: AvisoFilters): Promise<{ avisos: AvisoRow[]; total: number }> {
    ...
    static async getStats(): Promise<AvisoStats> {
        const query = `
            SELECT
                COUNT(*) as total,
    ...
    static async getById(id: number): Promise<AvisoRow | null> {
        const query = 'SELECT * FROM noticias WHERE id = $1';
        const result = await executeQuery(query, [id]);
        return result[0] || null;
    }

    static async getBySlug(slug: string): Promise<AvisoRow | null> {
        try {
    ...
    static async update(id: number, updateData: AvisoUpdateData): Promise<AvisoRow | null> {
        const { titulo, contenido, resumen, imagen_url, categoria, estado, destacada } = updateData;

        let publicoVal: boolean | null = null;
