...
class NotificacionesConvocatoriasDAO {
    static async getByEmail(email) {
        const result = await (0, database_1.executeQuery)('SELECT id, status FROM notificaciones_convocatorias WHERE email = $1', [email]);
    ...
    static async reactivate(email, data) {
        ...
        `;
        const result = await (0, database_1.executeQuery)(query, [data.nombre, data.tipo_interes, data.ip_address, data.user_agent, email]);
        return result[0];
    }
    static async create(data) {
        ...
        `;
        const result = await (0, database_1.executeQuery)(query, [data.nombre, data.email, data.tipo_interes, data.ip_address, data.user_agent]);
        return result[0];
    }
    ...
    static async getStats() {
        ...
        `;
        const tipoResult = await (0, database_1.executeQuery)(tipoQuery, []);
    ...
    static async getById(id) {
        const result = await (0, database_1.executeQuery)('SELECT * FROM notificaciones_convocatorias WHERE id = $1', [id]);
        return result[0] || null;
    }
    static async update(id, data) {
        ...
        `;
        const result = await (0, database_1.executeQuery)(query, [data.nombre, data.tipo_interes, data.status, id]);
        return result[0] || null;
    }
...
exports.default = NotificacionesConvocatoriasDAO;
module.exports = NotificacionesConvocatoriasDAO;
//# sourceMappingURL=notificaciones-convocatorias.dao.js.map