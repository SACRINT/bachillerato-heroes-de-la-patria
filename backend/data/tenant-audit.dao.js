...
class TenantAuditDAO {
    ...
    static async logEvent(tenantId, userId, eventType, action, entityType, entityId, oldValue, newValue, ipAddress, userAgent, metadata) {
        try {
            ...
            `, [tenantId, userId, eventType, action, entityType, entityId, oldValue ? JSON.stringify(oldValue) : null, newValue ? JSON.stringify(newValue) : null, ipAddress, userAgent, JSON.stringify(metadata || {})]);
    ...
    static async getLogs(tenantId, filters) {
        try {
            ...
            let query = 'SELECT * FROM audit_log WHERE tenant_id = $1'; const params = [tenantId]; let idx = 2;
            ...
            return result.rows;
        } catch (error) { console.error('[AUDIT-DAO] Error obteniendo logs:', error.message); return []; }
    }
}
