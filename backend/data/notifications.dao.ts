...
class NotificationDAO {
    ...
    static async create(data: NotificationCreateData): Promise<NotificationRow> {
        const query = `
    ...
    static async get(id: number): Promise<NotificationRow | undefined> {
        const query = `SELECT * FROM notificaciones WHERE id = $1`;
        const result = await executeQuery(query, [id]);
        return result[0];
    }

    static async list(filters: NotificationFilters = {}): Promise<NotificationRow[]> {
    ...
    static async getByUser(userId: number, filters: NotificationFilters = {}): Promise<NotificationRow[]> {
        return await this.list({ ...filters, usuario_id: userId });
    }

    static async markAsRead(id: number, userId: number): Promise<NotificationRow | undefined> {
        const query = `
