...
class SolicitudesDocumentosDAO {
    ...
    static async create(data: CreateSolicitudInput): Promise<SolicitudDocumento> {
        const query = `
    ...
    static async getAll({ status, tipo_usuario, nivel_urgencia, limit = 50, offset = 0 }: SolicitudFilter): Promise<SolicitudListResult> {
        ...
        return { data: data as SolicitudDocumento[], total: parseInt(countResult[0].count) };
    }

    static async getStats(): Promise<SolicitudStats> {
        const query = `
        ...
        const byTipoUsuario = tipoResult.reduce((acc: any, row: any) => { acc[row.tipo_usuario] = parseInt(row.cantidad); return acc; }, {});
        ...
        const documentosMasSolicitados = docResult.reduce((acc: any, row: any) => { acc[row.documento_solicitado] = parseInt(row.cantidad); return acc; }, {});
    ...
    static async getById(id: number): Promise<SolicitudDocumento | null> {
        const result = await executeQuery('SELECT * FROM solicitudes_documentos WHERE id = $1', [id]);
        return result[0] || null;
    }

    static async update(id: number, data: Partial<SolicitudDocumento>): Promise<SolicitudDocumento | null> {
        const query = `
    ...
    static async approve(id: number, notas_admin?: string): Promise<SolicitudDocumento | null> {
    ...
    static async reject(id: number, motivo: string): Promise<SolicitudDocumento | null> {
