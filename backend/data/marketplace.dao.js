...
class MarketplaceDAO {
    ...
    static async createItem(data) {
        const { sellerId, categoryId, title, slug, description, shortDescription, itemType, contentUrl, previewUrl, fileType, fileSizeBytes, priceCoins, subject, topics, gradeLevel, tags, metadata } = data;
    ...
    static async ensureCreatorEarnings(sellerId) {
        await database_1.pool.query(`INSERT INTO creator_earnings (creator_id) VALUES ($1) ON CONFLICT (creator_id) DO NOTHING`, [sellerId]);
    }
    ...
    static async incrementViewCount(itemId) {
        await database_1.pool.query(`UPDATE marketplace_items SET view_count = view_count + 1 WHERE id = $1`, [itemId]);
    }
    static async checkPurchase(userId, itemId) {
        const result = await database_1.pool.query(`SELECT * FROM marketplace_purchases WHERE buyer_id = $1 AND item_id = $2`, [userId, itemId]);
        return result.rows.length > 0;
    }
    static async checkFavorite(userId, itemId) {
        const result = await database_1.pool.query(`SELECT id FROM marketplace_favorites WHERE user_id = $1 AND item_id = $2`, [userId, itemId]);
        return result.rows.length > 0;
    }
    static async getItemSeller(itemId) {
        const result = await database_1.pool.query(`SELECT seller_id FROM marketplace_items WHERE id = $1`, [itemId]);
    ...
    static async submitForReview(itemId, sellerId) {
        const result = await database_1.pool.query(`UPDATE marketplace_items SET status = 'pending_review', updated_at = NOW() WHERE id = $1 AND seller_id = $2 RETURNING *`, [itemId, sellerId]);
        return result.rows[0];
    }
    ...
    static async hasAccess(userId, itemId) {
        const result = await database_1.pool.query(`SELECT id FROM marketplace_purchases WHERE buyer_id = $1 AND item_id = $2`, [userId, itemId]);
        return result.rows.length > 0;
    }
    // ==========================================
    // REVIEWS
    ...
    static async addToFavorites(userId, itemId) {
        const result = await database_1.pool.query(`INSERT INTO marketplace_favorites (user_id, item_id) VALUES ($1, $2) ON CONFLICT (user_id, item_id) DO NOTHING RETURNING *`, [userId, itemId]);
        return result.rows[0];
    }
    static async removeFromFavorites(userId, itemId) {
        await database_1.pool.query(`DELETE FROM marketplace_favorites WHERE user_id = $1 AND item_id = $2`, [userId, itemId]);
    }
    ...
    static async getCreatorEarnings(creatorId) {
        const result = await database_1.pool.query(`SELECT * FROM creator_earnings WHERE creator_id = $1`, [creatorId]);
        return result.rows[0];
    }
    static async getCreatorTransactions(creatorId, limit, offset) {
        const result = await database_1.pool.query(`SELECT * FROM creator_transactions WHERE creator_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`, [creatorId, limit, offset]);
        return result.rows;
    }
    ...
    static async getCategories(parentId) {
        ...
        const result = await database_1.pool.query(query, parentId ? [parentId] : []);
        return result.rows;
    }
    ...
    static async reportItem(itemId, reporterId, reason, description) {
        const result = await database_1.pool.query(`INSERT INTO marketplace_reports (item_id, reporter_id, reason, description) VALUES ($1, $2, $3, $4) RETURNING *`, [itemId, reporterId, reason, description]);
        return result.rows[0];
    }
...
exports.default = MarketplaceDAO;
module.exports = MarketplaceDAO;
//# sourceMappingURL=marketplace.dao.js.map