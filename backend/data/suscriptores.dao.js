...
class SuscriptoresDAO {
    ...
    static async getByEstado(estado) {
        return (await (0, database_1.executeQuery)('SELECT * FROM suscriptores_notificaciones WHERE estado = $1 ORDER BY fecha_registro DESC', [estado]));
    }
    static async getActivosForEmail(tipo) {
    ...
    static async getStats() {
        const total = await (0, database_1.executeQuery)('SELECT COUNT(*) as total FROM suscriptores_notificaciones', []);
        const porEstado = await (0, database_1.executeQuery)('SELECT estado, COUNT(*) as cantidad FROM suscriptores_notificaciones GROUP BY estado', []);
        const porVerificacion = await (0, database_1.executeQuery)('SELECT verificado, COUNT(*) as cantidad FROM suscriptores_notificaciones GROUP BY verificado', []);
        const porTipo = await (0, database_1.executeQuery)(`
            SELECT
        ...
        `, []);
        const nuevos = await (0, database_1.executeQuery)(`SELECT COUNT(*) as total FROM suscriptores_notificaciones WHERE fecha_registro >= NOW() - INTERVAL '7 days'`, []);
        const tasaApertura = await (0, database_1.executeQuery)(`
        ...
        return {
            ...
            porEstado: porEstado.map((r) => ({ estado: r.estado, cantidad: parseInt(r.cantidad) })),
            porVerificacion: porVerificacion.map((r) => ({ verificado: r.verificado, cantidad: parseInt(r.cantidad) })),
            porTipo: {
    ...
    static async getById(id) {
        const result = await (0, database_1.executeQuery)('SELECT * FROM suscriptores_notificaciones WHERE id = $1', [id]);
        return result[0] || null;
    }
    static async getByEmail(email) {
        const result = await (0, database_1.executeQuery)('SELECT id, estado FROM suscriptores_notificaciones WHERE email = $1', [email]);
        return result[0] || null;
    }
    ...
    static async reactivate(email, prefs) {
        const query = `
    ...
    static async updatePreferences(email, prefs) {
        const query = `
            UPDATE suscriptores_notificaciones SET
    ...
    static async registerSend(id, abierto = false) {
        ...
        return await (0, database_1.executeQuery)(`UPDATE suscriptores_notificaciones SET ${updateFields} WHERE id = $1`, [id]);
    }
    static async delete(id) {
        return await (0, database_1.executeQuery)('DELETE FROM suscriptores_notificaciones WHERE id = $1', [id]);
    }
}
exports.default = SuscriptoresDAO;
module.exports = SuscriptoresDAO;
//# sourceMappingURL=suscriptores.dao.js.map