...
export interface Webhook {
    ...
    status?: string; // Tenant-aware ('active', 'inactive')
    metadata?: any;
    description?: string;
...
export interface WebhookDelivery {
    id: number;
    webhook_id: number;
    event_type: string; // or 'event' in legacy
    event?: string; // Legacy
    payload?: any;
    success?: boolean; // Legacy
    status?: string;
    status_code?: number;
    response_code?: number; // Tenant-aware
    error?: string; // Legacy
...
class WebhookDAO {
    ...
    static async register(url: string, events: string[], secret: string, metadata: any): Promise<Webhook> {
        const result = await pool.query(
            'INSERT INTO webhooks (url, events, secret, active, metadata, created_at) VALUES ($1, $2, $3, true, $4, NOW()) RETURNING id, url, events, secret',
            [url, JSON.stringify(events), secret, JSON.stringify(metadata)]
        );
        return result.rows[0];
    }

    static async findByEvent(event: string): Promise<Webhook[]> {
        const result = await pool.query(
            'SELECT * FROM webhooks WHERE active = true AND events::jsonb ? $1',
            [event]
        );
        return result.rows;
    }

    static async logDelivery(webhookId: number, event: string, success: boolean, statusCode: number, error: string): Promise<void> {
        try {
            await pool.query(
                'INSERT INTO webhook_deliveries (webhook_id, event, success, status_code, error, created_at) VALUES ($1, $2, $3, $4, $5, NOW())',
    ...
    static async toggle(id: number, active: boolean): Promise<void> {
     ...
     */
    static async listByTenant(tenantId: number): Promise<Webhook[]> {
        const result = await pool.query(
     ...
     */
    static async createForTenant(tenantId: number, url: string, events: string[], description: string, secret: string, secretPreview: string): Promise<Webhook> {
        const result = await pool.query(
     ...
     */
    static async getByIdAndTenant(id: number, tenantId: number): Promise<Webhook | null> {
        const result = await pool.query(
     ...
     */
    static async deleteByIdAndTenant(id: number, tenantId: number): Promise<{ id: number } | null> {
        const result = await pool.query(
     ...
     */
    static async belongsToTenant(id: number, tenantId: number): Promise<boolean> {
        const result = await pool.query(
     ...
     */
    static async getDeliveries(webhookId: number, limit: number = 50, offset: number = 0): Promise<WebhookDelivery[]> {
        const result = await pool.query(
     ...
     */
    static async getWithSecret(id: number, tenantId: number): Promise<Webhook | null> {
        const result = await pool.query(
     ...
     */
    static async logDeliveryFull(webhookId: number, eventType: string, payload: any, status: string, responseCode: number, responseBody: string | null, retryCount: number): Promise<void> {
        await pool.query(
     ...
     */
    static async logDeliveryError(webhookId: number, eventType: string, payload: any, errorMessage: string, retryCount: number): Promise<void> {
        await pool.query(
            `INSERT INTO webhook_deliveries
     ...
     */
    static async getActiveForEvent(tenantId: number, eventType: string): Promise<Webhook[]> {
        const result = await pool.query(
     ...
     */
    static async updateByIdAndTenant(id: number, tenantId: number, updates: WebhookUpdates): Promise<Webhook | null> {
