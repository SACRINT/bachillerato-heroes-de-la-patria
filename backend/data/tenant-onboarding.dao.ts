...
class TenantOnboardingDAO {
    ...
    static async checkSubdomainExists(subdomain: string): Promise<boolean> {
    ...
    static async checkDomainExists(domain: string): Promise<boolean> {
        const result = await pool.query('SELECT id FROM tenants WHERE domain = $1', [domain]);
        return result.rows.length > 0;
    }
    ...
    static async createTenantWithAdmin(client: any, tenantData: TenantData, adminData: AdminData, seedData: boolean): Promise<TenantCreationResult> {
    ...
    static async updateStatus(tenantId: number, status: string): Promise<any> {
        const result = await pool.query('UPDATE tenants SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING *', [status, tenantId]);
        return result.rows[0];
    }

    static async updateConfig(tenantId: number, newConfig: any): Promise<any> {
        const result = await pool.query('UPDATE tenants SET config_json = config_json || $1::jsonb, updated_at = NOW() WHERE id = $2 RETURNING *', [JSON.stringify(newConfig), tenantId]);
        return result.rows[0];
    }
