...
class AppointmentDAO {
    ...
    static async create(data: AppointmentCreateData): Promise<AppointmentRow> {
        const query = `
            INSERT INTO citas (
    ...
    static async get(id: number | string): Promise<AppointmentRow | undefined> {
    ...
    static async getByToken(token: string): Promise<AppointmentRow | undefined> {
        const query = `SELECT * FROM citas WHERE token_confirmacion = $1`;
        const result = await executeQuery(query, [token]);
        return result[0];
    }

    static async update(id: number, data: AppointmentUpdateData): Promise<AppointmentRow | null> {
    ...
    static async list(filters: AppointmentFilters = {}): Promise<AppointmentRow[]> {
    ...
    static async checkAvailability(fecha: Date | string, hora: string): Promise<boolean> {
        const query = `
    ...
    static async countByUserAndDate(email: string, fecha: Date | string): Promise<number> {
        const query = `
            SELECT COUNT(*) as count 
            FROM citas 
            WHERE email = $1 
    ...
    static async getLastCitaId(): Promise<string | null> {
