...
class BolsaTrabajoDAO {
    static async createPendingConfirmation(email, formData, token) {
        ...
        const result = await (0, database_1.executeQuery)(query, [email, JSON.stringify(formData), token]);
        return result[0];
    }
    static async getPendingByToken(token) {
        const result = await (0, database_1.executeQuery)('SELECT id, email_usuario, datos_json, token_expires_at FROM bolsa_trabajo_pending_confirmation WHERE confirmation_token = $1', [token]);
        return result[0] || null;
    }
    static async deletePendingById(id) {
        await (0, database_1.executeQuery)('DELETE FROM bolsa_trabajo_pending_confirmation WHERE id = $1', [id]);
    }
    ...
    static async getCvs(filters) {
        ...
        const countResult = await (0, database_1.executeQuery)(cq, status ? [status] : []);
    ...
    static async getCvStats() {
        ...
        const yearResult = await (0, database_1.executeQuery)('SELECT anio_egreso, COUNT(*) as cantidad FROM bolsa_trabajo GROUP BY anio_egreso ORDER BY anio_egreso DESC', []);
        const byYear = yearResult.reduce((acc, row) => { acc[row.anio_egreso] = parseInt(row.cantidad); return acc; }, {});
        const areaResult = await (0, database_1.executeQuery)('SELECT area_interes, COUNT(*) as cantidad FROM bolsa_trabajo WHERE area_interes IS NOT NULL GROUP BY area_interes ORDER BY cantidad DESC LIMIT 10', []);
    ...
    static async getCvById(id) {
        const result = await (0, database_1.executeQuery)('SELECT * FROM bolsa_trabajo WHERE id = $1', [id]);
        return result[0] || null;
    }
    static async updateCv(id, data) {
        ...
        const result = await (0, database_1.executeQuery)(query, [data.nombre, data.email, data.telefono, data.anio_egreso, data.area_interes, data.resumen_profesional, data.habilidades, data.status, id]);
        return result[0] || null;
    }
    static async deleteCv(id) {
        const result = await (0, database_1.executeQuery)('DELETE FROM bolsa_trabajo WHERE id = $1 RETURNING id', [id]);
        return result[0] || null;
    }
    static async getAll(filters) {
        ...
        const countResult = await (0, database_1.executeQuery)(cq, estado ? [estado] : []);
        return { data, total: parseInt(countResult[0].count) };
    }
    static async getGeneralStats() {
        ...
        const result = await (0, database_1.executeQuery)(query, []);
        const yearResult = await (0, database_1.executeQuery)('SELECT generacion, COUNT(*) as cantidad FROM bolsa_trabajo WHERE generacion IS NOT NULL GROUP BY generacion ORDER BY generacion DESC', []);
        const byYear = yearResult.reduce((acc, row) => { acc[row.generacion] = parseInt(row.cantidad); return acc; }, {});
        const expResult = await (0, database_1.executeQuery)(`SELECT CASE WHEN experiencia IS NULL OR experiencia = '' THEN 'Sin experiencia' ELSE 'Con experiencia' END as tipo_experiencia, COUNT(*) as cantidad FROM bolsa_trabajo GROUP BY tipo_experiencia ORDER BY cantidad DESC`, []);
    ...
    static async getPendingApprovals(filters) {
        ...
        if (email_confirmado !== undefined && email_confirmado !== 'undefined') {
            cq += ` AND email_confirmado = $${++cpc}`;
            cp.push(email_confirmado === 'true' || email_confirmado === true);
        }
        const countResult = await (0, database_1.executeQuery)(cq, cp);
        return { data, total: parseInt(countResult[0].count) };
    }
    static async getSolicitudById(id) {
        const result = await (0, database_1.executeQuery)(`SELECT id, uuid, email_usuario, datos_json, estado, tipo_solicitud FROM pendientes_aprobacion WHERE id = $1 AND tipo_solicitud = 'bolsa_trabajo'`, [id]);
        return result[0] || null;
    }
    static async updateSolicitudStatus(id, estado, adminNotas, adminId) {
        const result = await (0, database_1.executeQuery)(`UPDATE pendientes_aprobacion SET estado = $1, admin_notas = $2, admin_id = $3, fecha_procesado = NOW() WHERE id = $4 RETURNING id, uuid, estado, email_usuario`, [estado, adminNotas || null, adminId || null, id]);
        return result[0] || null;
    }
    static async insertCvFromApproval(formData) {
        const result = await (0, database_1.executeQuery)(`INSERT INTO bolsa_trabajo (nombre, email, telefono, anio_egreso, area_interes, resumen_profesional, habilidades, estado, verificado, fecha_creacion) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW()) RETURNING id, uuid`, [formData.name, formData.email, formData.phone, formData.graduationYear, formData.subject, formData.message, formData.skills ? JSON.stringify(formData.skills) : null, 'activo', true]);
        return result[0];
    }
}
exports.default = BolsaTrabajoDAO;
module.exports = BolsaTrabajoDAO;
//# sourceMappingURL=bolsa-trabajo.dao.js.map